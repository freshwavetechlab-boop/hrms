...
import { createQueryPlan, repairQueryPlan, createExecutiveInsights } from './analytics/planner.mjs'
...
import { scoreCatalogTable, relevantRelationshipHints } from './analytics/retrieval.mjs'
...
async function run(input) {
  try {
    ...
    const catalog = input.catalog.map(table => ({ ...table, score: scoreCatalogTable(input.question, table) }))
    ...
    const chart = buildChart(rows, { ...plan, presentationMode: 'dashboard' })
    stage(92, 'Checking the result summary against returned data')
    const narrative = await createExecutiveInsights({ question: input.question, rows, chart, knowledge,
      localProvider: String(plan.providerCode || '').toLowerCase() === 'localopenaicompatible' })
