...
import { Alert, Button, Card, Drawer, Empty, Form, Input, InputNumber, Modal, Select, Space, Statistic, Tag, Timeline, message } from 'antd'
import { ClockCircleOutlined, FileProtectOutlined, PauseCircleOutlined, PlayCircleOutlined, PlusOutlined, TeamOutlined } from '@ant-design/icons'
...
export default function RecruitmentWorkOrderWorkspace() {
  ...
  const pause = () => {
    ...
    let reason = ''
    Modal.confirm({ title: 'Pause this stage SLA?', content: <Input.TextArea data-testid="hiring-case-pause-reason" autoFocus placeholder="Mandatory business reason" onChange={event => { reason = event.target.value }} />, okText: 'Pause SLA', onOk: async () => {
      if (reason.trim().length < 3) { message.warning('Enter the pause reason.'); return Promise.reject() }
      const response = await pauseRecruitmentHiringCase(selectedCase.id, reason)
      if (response.ok && response.data) { setSelectedCase(response.data); await load() }
    } })
  }
  const resume = async () => {
    if (!selectedCase) return
  ...
  const advance = () => {
    if (!selectedCase) return
    let reason = ''
    Modal.confirm({ title: 'Move to the next configured stage?', content: <Input.TextArea placeholder="Movement note (optional)" onChange={event => { reason = event.target.value }} />, okText: 'Move stage', onOk: async () => {
      const response = await advanceRecruitmentHiringCase(selectedCase.id, reason)
  ...
  return <section className="work-order-workspace" data-testid="recruitment-work-orders">
    <div className="work-order-command-bar">
      <div><span>Client hiring control tower</span><h2>Work Orders & SLA Cases</h2><p>Register the client demand manually, secure its annexures and run one auditable cumulative timeline before candidates branch into their individual pipelines.</p></div>
      <Space wrap><Select allowClear value={clientId || undefined} placeholder="All accessible clients" showSearch optionFilterProp="label" options={clients.map(client => ({ value: client.id, label: client.name }))} onChange={value => setClientId(value || 0)} /><Input.Search value={query} placeholder="Work order or subject" onChange={event => setQuery(event.target.value)} onSearch={() => void load()} /><Button data-testid="work-order-add" type="primary" size="large" icon={<PlusOutlined />} onClick={openNew}>Add work order</Button></Space>
    </div>
    <div className="work-order-metrics">
      <Card><Statistic title="Active work orders" value={stats.activeOrders} prefix={<FileProtectOutlined />} /></Card>
      <Card><Statistic title="Position lines" value={stats.positions} prefix={<TeamOutlined />} /></Card>
      <Card><Statistic title="Live hiring cases" value={stats.activeCases} prefix={<PlayCircleOutlined />} /></Card>
      <Card className={stats.breached ? 'risk' : ''}><Statistic title="Overall SLA breached" value={stats.breached} prefix={<ClockCircleOutlined />} /></Card>
    </div>
    <div className="work-order-columns">
      <Card title="Client work orders" extra={<Tag>{workOrders.length} records</Tag>}>
        {!workOrders.length ? <Empty description="No work order has been entered for this client." /> : <div className="work-order-list">{workOrders.map(row => <article key={row.id}>
          ...
          <footer><span>{dateTimeText(row.receivedAtUtc)}</span><b>{row.lineCount} position line{row.lineCount === 1 ? '' : 's'}</b><span>{durationText(row.overallSlaMinutes)} overall</span><Button size="small" onClick={() => void openEdit(row)}>Edit</Button></footer>
        </article>)}</div>}
      </Card>
      <Card title="Live cumulative pipelines" extra={<Tag color="purple">Position scope</Tag>}>
        {!cases.length ? <Empty description="Start a published position pipeline from a work-order line." /> : <div className="hiring-case-list">{cases.map(row => {
    ...
    <Drawer width={860} title={draft.id ? `Edit ${draft.workOrderNumber}` : 'New client work order'} open={editorOpen} onClose={() => setEditorOpen(false)} extra={<Button data-testid="work-order-save" type="primary" loading={saving} onClick={() => void save()}>Save work order</Button>}>
      ...
      <Form layout="vertical" className="work-order-form">
        ...
        <div className="work-order-line-heading"><div><h3>Position lines</h3><p>Each role can start one independently tracked position pipeline.</p></div><Button data-testid="work-order-add-position" icon={<PlusOutlined />} onClick={addLine}>Add position</Button></div>
    ...
    <Drawer width={980} title={selectedWorkOrder ? `${selectedWorkOrder.workOrderNumber} · ${selectedWorkOrder.clientName}` : 'Work order'} open={!!selectedWorkOrder} onClose={() => setSelectedWorkOrder(null)}>
      {selectedWorkOrder && <><div className="work-order-detail-strip"><div><span>Received</span><b>{dateTimeText(selectedWorkOrder.receivedAtUtc)}</b></div><div><span>Overall due</span><b>{dateTimeText(selectedWorkOrder.dueAtUtc)}</b></div><div><span>Source</span><b>{selectedWorkOrder.receivedFrom || 'Manual entry'}</b></div><Tag color={statusColor(selectedWorkOrder.status)}>{selectedWorkOrder.status}</Tag></div>
        <div className="work-order-detail-lines">{selectedWorkOrder.lines.map(line => <article key={line.id}><div><span>Line {line.lineNumber}</span><h3>{line.positionName}</h3><p>{[line.payBandLevelCode, line.division, line.location].filter(Boolean).join(' · ') || 'Position metadata not entered'}</p></div><div><Tag>{line.numberOfPositions} position{line.numberOfPositions === 1 ? '' : 's'}</Tag><Button type="primary" disabled={cases.some(row => row.workOrderLineId === line.id)} onClick={() => void prepareStart(selectedWorkOrder, line.id)}>{cases.some(row => row.workOrderLineId === line.id) ? 'Case started' : 'Start pipeline'}</Button></div></article>)}</div>
    ...
    <Drawer width={1040} title={selectedCase ? `${selectedCase.positionName} · ${selectedCase.workOrderNumber}` : 'Hiring case'} open={!!selectedCase} onClose={() => { setSelectedCase(null); setProcessDocuments([]); setProfileBatches([]); setCandidateApplications([]); setSelectedApplicationIds([]) }} extra={selectedCase?.status === 'Active' && <Space>{activeStage?.isPaused ? <Button icon={<PlayCircleOutlined />} onClick={() => void resume()}>Resume SLA</Button> : <Button icon={<PauseCircleOutlined />} onClick={pause}>Pause SLA</Button>}<Button type="primary" disabled={selectedCase.advanceStatus === 'Pending Approval'} onClick={advance}>{selectedCase.advanceStatus === 'Pending Approval' ? 'Approval pending' : 'Move stage'}</Button></Space>}>
