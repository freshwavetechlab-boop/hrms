...
const workingHoursOptions: AttendanceSettings['workingHoursCalculation'][] = ['First check-in and last check-out', 'Every valid check-in and check-out']
const regularizationWindowOptions: AttendanceSettings['regularizationWindow'][] = ['Anytime', 'Limited by past days']
...
export default function AttendanceSettingsForm({ clientId, onSaved }: { clientId: number; onSaved: (message: string) => void }) {
  ...
  const set = <K extends keyof AttendanceSettings>(key: K, value: AttendanceSettings[K]) => {
    setErrors([])
    setForm(current => {
      if (key === 'allowRegularizationRequests' && !value) return { ...current, allowRegularizationRequests: false, regularizationWindow: 'Anytime', restrictRegularizationRequestsPerMonth: false } as AttendanceSettings
      if (key === 'restrictRegularizationRequestsPerMonth' && !value) return { ...current, restrictRegularizationRequestsPerMonth: false } as AttendanceSettings
  ...
  return <section className="card attendance-settings"><header><i className="blue">A</i><div><h3>Attendance Management</h3><p>Configure shift times, workday duration and regularization controls.</p></div></header>{errors.length > 0 && <div className="form-errors sticky-errors">{errors.map(error => <p key={error}>{error}</p>)}</div>}<h3>Work Shift Time</h3><div className="grid"><label><span>Check-in time</span><input type="time" value={timeValue(form.checkInTime)} onChange={event => set('checkInTime', `${event.target.value}:00`)} /></label><label><span>Check-out time</span><input type="time" value={timeValue(form.checkOutTime)} onChange={event => set('checkOutTime', `${event.target.value}:00`)} /></label></div><h3>Working Hours Calculation</h3><div className="grid"><label className="wide leave-dropdown-field"><span>Calculation method</span><div className="leave-select-shell"><select value={form.workingHoursCalculation} onChange={event => set('workingHoursCalculation', event.target.value as AttendanceSettings['workingHoursCalculation'])}>{workingHoursOptions.map(option => <option value={option} key={option}>{option}</option>)}</select></div></label></div><h3>Workday Duration</h3><div className="grid"><label><span>Minimum hours for half-day</span><input type="number" step="0.25" value={form.minimumHoursForHalfDay} onChange={event => set('minimumHoursForHalfDay', Number(event.target.value))} /></label><label><span>Minimum hours for full-day</span><input type="number" step="0.25" value={form.minimumHoursForFullDay} onChange={event => set('minimumHoursForFullDay', Number(event.target.value))} /></label><label><span>Maximum hours allowed for full-day</span><input type="number" step="0.25" value={form.maximumHoursAllowedForFullDay} onChange={event => set('maximumHoursAllowedForFullDay', Number(event.target.value))} /></label></div><h3>Regularization Settings</h3><div className="grid"><Check label="Allow regularization requests" value={form.allowRegularizationRequests} set={value => set('allowRegularizationRequests', value)} /><label className="leave-dropdown-field"><span>Request window</span><div className="leave-select-shell"><select value={form.regularizationWindow} disabled={!form.allowRegularizationRequests} onChange={event => set('regularizationWindow', event.target.value as AttendanceSettings['regularizationWindow'])}>{regularizationWindowOptions.map(option => <option value={option} key={option}>{option}</option>)}</select></div></label>{form.regularizationWindow === 'Limited by past days' && <label><span>Number of past days allowed</span><input type="number" disabled={!form.allowRegularizationRequests} value={form.pastDaysAllowed} onChange={event => set('pastDaysAllowed', Number(event.target.value))} /></label>}<Check label="Restrict regularization requests per month" value={form.restrictRegularizationRequestsPerMonth} set={value => set('restrictRegularizationRequestsPerMonth', value)} />{form.restrictRegularizationRequestsPerMonth && <label><span>Max requests per month</span><input type="number" disabled={!form.allowRegularizationRequests} value={form.maxRegularizationRequestsPerMonth} onChange={event => set('maxRegularizationRequestsPerMonth', Number(event.target.value))} /></label>}</div><div className="actions"><p>Rules are applied during attendance calculation and payroll cut-off processing.</p><button type="button" disabled={saving} onClick={() => void save()}>{saving ? 'Saving...' : 'Save attendance settings'}</button></div></section>
}
...
function normalizeTimes(settings: AttendanceSettings) {
  const workingHoursCalculation = normalizeOption(settings.workingHoursCalculation, workingHoursOptions, initial.workingHoursCalculation)
  const regularizationWindow = normalizeOption(settings.regularizationWindow, regularizationWindowOptions, initial.regularizationWindow)
  return { ...initial, ...settings, workingHoursCalculation, regularizationWindow, checkInTime: settings.checkInTime?.slice(0, 8) || initial.checkInTime, checkOutTime: settings.checkOutTime?.slice(0, 8) || initial.checkOutTime }
}
