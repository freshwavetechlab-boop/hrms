  ...
  return <div className="engine-chart-card" data-testid="engine-history-panel">
    ...
    {period !== 'live' && <>
      ...
      {data && <>
        ...
        <div className="engine-chart-controls"><Tag color="blue">{total} completed operations</Tag><Tag color={failures ? 'error' : 'green'}>{failures} failed attempts</Tag><Segmented<EngineChartMetric> value={metric} onChange={setMetric} options={[{ value: 'requests', label: 'Completed work' }, { value: 'busy', label: 'Busy time (%)' }]} /></div>
