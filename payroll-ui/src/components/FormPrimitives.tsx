...
export function Sel(p: { v: string | number; set: (v: string) => void; a: string[] }) { return <SearchSelect value={p.v} onChange={p.set} options={[{ value: '', label: 'Select' }, ...p.a.map(parseOption)]} /> }
...
function parseOption(item: string) {
  const match = item.match(/^(\d+)[.:](.+)$/)
  if (match) return { value: match[1], label: match[2] }
  const [value, ...label] = item.split(':')
  return { value: label.length ? value : item, label: label.join(':') || item }
}
