...
export default function SecurityPanel({ initialTab = 'Users' }: { initialTab?: 'Users' | 'Roles' | 'Audit' }) {
  ...
  const saveUser = async () => {
    ...
    try {
      ...
      if (response.ok) { if (user.id === 0 && user.employeeId) setCreatedCredentials({ email: body.email, password: body.password }); setUser(user0); await load() }
    } catch {
