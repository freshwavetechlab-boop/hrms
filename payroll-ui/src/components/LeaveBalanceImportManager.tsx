...
export default function LeaveBalanceImportManager({ onMessage }: { onMessage: (message: string) => void }) {
  ...
  const upload = async (manualMapping?: LeaveBalanceImportMapping) => {
    ...
    const response = await previewLeaveBalanceImport(file, encoding, manualMapping)
  ...
  const importRows = async () => {
    if (!preview) return
    setBusy(true); setError('')
    const response = await finalizeLeaveBalanceImport(preview, encoding)
  ...
  const downloadSample = async () => { const response = await fetch(leaveBalanceSampleUrl()); const blob = await response.blob(); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'leave-balance-import-sample.csv'; link.click(); URL.revokeObjectURL(url) }
...
function RecordTable(p: { rows: LeaveBalanceImportPreview['validRecords']; showErrors?: boolean }) {
  return <div className="import-table"><table><thead><tr><th>Row</th><th>Employee Number</th><th>Leave Type</th><th>Date</th><th>Count</th>{p.showErrors && <th>Errors</th>}</tr></thead><tbody>{p.rows.slice(0, 50).map(row => <tr className={row.isValid ? '' : 'error'} key={row.rowNumber}><td>{row.rowNumber}</td><td>{row.employeeNumber}</td><td>{row.leaveType}</td><td>{row.date}</td><td>{row.count}</td>{p.showErrors && <td>{row.errors.join('; ')}</td>}</tr>)}</tbody></table>{!p.rows.length && <p className="empty">No records.</p>}{p.rows.length > 50 && <p className="empty">Showing first 50 records.</p>}</div>
}
