...
const category0: TravelExpenseCategory = { id: 0, parentId: null, parentName: '', categoryCode: '', categoryName: '', receiptMandatory: false, gstApplicable: false, dailyLimit: null, maximumClaim: null, requiresFinanceApproval: false, requiresManagerApproval: false, isActive: true }
...
export default function TravelExpensePolicySettings() {
  ...
  const openCategory = (row?: TravelExpenseCategory) => { setCategory(row ? { ...category0, ...row, parentId: row.parentId ?? null, dailyLimit: row.dailyLimit ?? null, maximumClaim: row.maximumClaim ?? null } : category0); setDrawer('category') }
  ...
  return <section className="travel-policy-settings">
    <Card title="Travel & Expense Policy Configuration" size="small" className="settings-panel settings-table-panel">
      <Tabs items={[
        ...
        { key: 'categories', label: 'Expense Categories', children: <><Header title="Expense category hierarchy" text="Reusable claim categories for future expense screens." action="Add category" onClick={() => openCategory()} /><DataTable rows={setup.categories} exportFileName="travel-expense-categories" actions={row => <Button size="small" type="primary" onClick={() => openCategory(row)}>Edit</Button>} columns={[
          { key: 'categoryCode', label: 'Code' },
          { key: 'categoryName', label: 'Category' },
          { key: 'parentName', label: 'Parent', value: row => row.parentName || '-' },
          { key: 'receiptMandatory', label: 'Receipt', value: row => row.receiptMandatory ? 'Mandatory' : 'Optional' },
          { key: 'gstApplicable', label: 'GST', value: row => row.gstApplicable ? 'Yes' : 'No' },
          { key: 'dailyLimit', label: 'Daily limit', value: row => row.dailyLimit ?? '-' },
          { key: 'maximumClaim', label: 'Max claim', value: row => row.maximumClaim ?? '-' },
          { key: 'isActive', label: 'Status', value: row => row.isActive ? 'Active' : 'Inactive' }
        ]} /></> },
    ...
    <Drawer className="settings-master-drawer travel-policy-drawer" title={drawerTitle(drawer)} open={drawer !== null} width="min(980px, 96vw)" destroyOnClose onClose={closeDrawer} footer={<Space><Button onClick={closeDrawer}>Cancel</Button><Button type="primary" onClick={() => drawer === 'policy' ? void savePolicyRow() : drawer === 'assignment' ? void saveAssignmentRow() : drawer === 'rule' ? void saveRuleRow() : void saveCategoryRow()}>Save</Button></Space>}>
      ...
      {drawer === 'category' && <Form layout="vertical" requiredMark={false}><Row gutter={12}>
        <Col xs={24} md={8}><Form.Item label="Category code"><Input value={category.categoryCode} onChange={event => setCategory({ ...category, categoryCode: event.target.value.toUpperCase() })} /></Form.Item></Col>
        <Col xs={24} md={16}><Form.Item label="Category name"><Input value={category.categoryName} onChange={event => setCategory({ ...category, categoryName: event.target.value })} /></Form.Item></Col>
        <Col xs={24} md={12}><Form.Item label="Parent category"><SearchSelect value={category.parentId || 0} onChange={value => setCategory({ ...category, parentId: Number(value) || null })} options={selectOptions(setup.categories.filter(item => item.id !== category.id).map(item => ({ value: item.id, label: item.categoryName })), 'No parent', 0)} /></Form.Item></Col>
