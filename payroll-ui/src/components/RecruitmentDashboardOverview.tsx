...
export default function RecruitmentDashboardOverview({
  ...
  const funnel = [
    ...
    { key: 'shortlisted', label: 'Shortlisted', value: shortlisted, conversion: pct(shortlisted, current.applications.length), onClick: () => onNavigate('/recruitment/ats-screening') },
  ...
  const charts = useMemo<DashboardChartSpec[]>(() => {
    ...
    return [
      ...
      { id: 'ta-ats-distribution', title: 'ATS score distribution', subtitle: 'Scoring coverage and candidate quality bands', ...metric(ats), kind: 'bar', compatibleKinds: ['bar', 'line', 'doughnut'], onPointClick: drill('/recruitment/ats-screening') },
