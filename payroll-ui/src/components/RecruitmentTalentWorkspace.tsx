...
function CandidateEditorDrawer({ draft, clients, onChange, onClose, onSubmit }: { draft: SaveRecruitmentCandidate | null; clients: Client[]; onChange: (value: SaveRecruitmentCandidate | null) => void; onClose: () => void; onSubmit: () => void }) {
  ...
  return <RecruitmentEditorDrawer open={!!draft} eyebrow="Talent profile" title={draft?.id ? 'Edit candidate' : 'Add candidate'} description="Maintain the candidate identity, work profile and consent details." onClose={onClose} onSubmit={onSubmit} submitText={draft?.id ? 'Save changes' : 'Add candidate'} width="min(850px, 96vw)">{draft && <Form layout="vertical" className="talent-form-grid">
    ...
    <Form.Item label="Current company"><Input value={draft.currentCompany} onChange={event => patch({ currentCompany: event.target.value })} /></Form.Item><Form.Item label="Current title"><Input value={draft.currentTitle} onChange={event => patch({ currentTitle: event.target.value })} /></Form.Item>
    <Form.Item label="Experience (months)"><InputNumber min={0} value={draft.totalExperienceMonths} onChange={value => patch({ totalExperienceMonths: Number(value || 0) })} /></Form.Item><Form.Item label="Current location"><Input value={draft.currentLocation} onChange={event => patch({ currentLocation: event.target.value })} /></Form.Item>
    <Form.Item label="Notice period (days)"><InputNumber min={0} value={draft.noticePeriodDays} onChange={value => patch({ noticePeriodDays: Number(value || 0) })} /></Form.Item><Form.Item label="Qualification"><Input value={draft.highestQualification} onChange={event => patch({ highestQualification: event.target.value })} /></Form.Item>
