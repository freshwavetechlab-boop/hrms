...
import { Button, Card, Checkbox, Drawer, Form, Input, InputNumber, Modal, Select, Space, Statistic, Tag } from 'antd'
...
import { changeApplicationStage, completeCandidateChecklistItem, convertCandidateToEmployee, createApplication, generateOfferLetter, getApplications, getCandidate, getCandidates, getInterviews, getOffers, getTalentDashboard, overrideApplicationScore, saveCandidate, saveCandidateProfileSections, saveOffer, scoreApplication, updateOfferStatus, uploadCandidateResume } from '../services/recruitmentTalentService'
...
export default function RecruitmentTalentWorkspace({ mode }: { mode: Mode }) {
  ...
  return <div className="talent-workspace">
    ...
    {mode === 'candidates' && <>
      ...
      <DataTable rows={candidates} exportFileName="talent-pool" actions={row => <Space><Button size="small" onClick={() => void openCandidate(row.id)}>360° profile</Button>{canApplyCandidate(row) && <Button size="small" type="primary" onClick={() => setApplicationDraft({ candidateId: row.id, positionId: 0, sourceType: 'Direct' })}>Apply</Button>}</Space>} columns={[
