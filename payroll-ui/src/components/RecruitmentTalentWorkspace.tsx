...
import RecruitmentNegotiationAction from './RecruitmentNegotiationAction'
...
import { Alert, Avatar, Button, Card, Checkbox, Drawer, Dropdown, Empty, Form, Input, InputNumber, Modal, Popconfirm, Select, Space, Tabs, Tag, Tooltip } from 'antd'
...
export default function RecruitmentTalentWorkspace({ mode, initialClientId = 0, negotiationMode = false }: { mode: Mode; initialClientId?: number; negotiationMode?: boolean }) {
  ...
  const [applicationQuickFilter, setApplicationQuickFilter] = useRecruitmentPreference<ApplicationQuickFilter>('applicationQuickFilter', 'all')
  const [applicationSearch, setApplicationSearch] = useRecruitmentPreference('applicationSearch', '')
  const [applicationPositionId, setApplicationPositionId] = useRecruitmentPreference('applicationPositionId', 0)
  const [applicationSource, setApplicationSource] = useRecruitmentPreference('applicationSource', '')
  const [applicationStage, setApplicationStage] = useRecruitmentPreference('applicationStage', '')
  const [applicationScoreBand, setApplicationScoreBand] = useRecruitmentPreference('applicationScoreBand', '')
  const [applicationSort, setApplicationSort] = useRecruitmentPreference('applicationSort', 'recent')
  const [batchInterviewOpen, setBatchInterviewOpen] = useState(false)
  const [batchInterviewApplicationIds, setBatchInterviewApplicationIds] = useRecruitmentPreference<number[]>('batchInterviewApplicationIds', [])
  ...
  const load = useCallback(async () => {
    ...
    if (mode === 'interviewQueue') {
      const [rows, rounds] = await Promise.all([getApplications(), getInterviews()])
      setApplications(initialClientId ? rows.filter(row => row.clientId === initialClientId) : rows)
      setInterviews(rounds)
      setBatchInterviewApplicationIds(ids => ids.filter(id => rows.some(row => row.id === id && interviewReady(row, rounds))))
      return
    }
    ...
    setCandidates(negotiationMode ? candidateRows.filter(row => applicationRows.some(application => application.candidateId === row.id)) : candidateRows); setApplications(applicationRows); setInterviews(interviewRows); setOffers(offerRows)
  ...
  const viewApplicationResume = async (applicationId: number) => {
    ...
    if (!application) {
      const interview = interviews.find(row => row.applicationId === applicationId)
      if (!interview?.candidateId) return void notify('No candidate resume is linked to this interview.', 'warning')
      const candidate = await getCandidate(interview.candidateId)
      const resume = candidate?.resumes.find(row => row.isPrimary) ?? candidate?.resumes[0]
      if (resume?.attachmentPublicId) await openAttachmentWithTicket(resume.attachmentPublicId, 'Preview')
      else notify('No resume is attached to this candidate.', 'warning')
      return
    }
  ...
  const createStandaloneInterviewApplication = async (input: { fullName: string; email: string; phone: string; positionId: number }) => {
    const response = await postJson('/api/recruitment/interviews/direct-application', input, null as RecruitmentCandidateApplication | null)
  ...
  const visibleApplications = useMemo(() => {
    ...
    const rows = intakeApplications.filter(row => {
      ...
      if (recordView !== 'Table' && (applicationPositionId && row.positionId !== applicationPositionId)) return false
      if (recordView !== 'Table' && (applicationSource === 'published' && !publicJobApplication(row))) return false
      if (recordView !== 'Table' && (applicationSource === 'manual' && publicJobApplication(row))) return false
      if (recordView !== 'Table' && (applicationStage && row.currentStage !== applicationStage)) return false
      if (recordView !== 'Table' && (applicationScoreBand === 'needs-score' && row.atsScore != null)) return false
      if (recordView !== 'Table' && (applicationScoreBand === 'below-60' && !(row.atsScore != null && row.atsScore < 60))) return false
      if (recordView !== 'Table' && (applicationScoreBand === '60-79' && !(row.atsScore != null && row.atsScore >= 60 && row.atsScore < 80))) return false
      if (recordView !== 'Table' && (applicationScoreBand === '80-plus' && !(row.atsScore != null && row.atsScore >= 80))) return false
      if (recordView !== 'Table' && (applicationQuickFilter === 'new' && !newApplication(row))) return false
      if (recordView !== 'Table' && (applicationQuickFilter === 'needs-score' && row.atsScore != null)) return false
      if (recordView !== 'Table' && (applicationQuickFilter === 'scored' && row.atsScore == null)) return false
      if (recordView !== 'Table' && (applicationQuickFilter === 'pipeline' && (newApplication(row) || rejectedApplication(row)))) return false
      if (recordView !== 'Table' && (applicationQuickFilter === 'rejected' && !rejectedApplication(row))) return false
      return true
    })
  ...
  }, [recordView, applicationPositionId, applicationQuickFilter, applicationScoreBand, applicationSearch, applicationSort, applicationSource, applicationStage, intakeApplications])
  ...
  const offerList = <RecruitmentRecordList key={negotiationMode ? 'negotiation' : 'offers'} rows={offers} title={row => row.candidateName} subtitle={row => row.positionTitle}
  ...
  if (String(mode) === 'applications') return <div className="talent-workspace">
    <section className="candidate-applications-workspace" data-testid="candidate-applications-workspace">
      <PageHeaderPortal slot="recruitment-page-controls"><Space wrap>{recordView !== 'Table' && applicationQuickFilter === 'rejected' && <Popconfirm title="Move selected candidates to Global Talent Pool?" description="Their rejected applications and complete pipeline history will remain linked to the original jobs." okText="Move selected" disabled={!selectedRejectedApplicationIds.length} onConfirm={() => void moveSelectedRejectedToGlobalTalentPool()}><Button disabled={!selectedRejectedApplicationIds.length}>Move selected to Global Talent Pool ({selectedRejectedApplicationIds.length})</Button></Popconfirm>}<AddCandidateMenu onSelect={target => { setResumeIntakeTarget(null); setResumeIntakeTalentPoolOnly(target === 'talentPool'); setResumeIntakeMode('single') }} /><Button type="primary" icon={<UploadOutlined />} onClick={() => { setResumeIntakeTalentPoolOnly(false); setResumeIntakeMode('bulk') }}>Bulk candidate upload</Button></Space></PageHeaderPortal>
    ...
    <Drawer className="candidate-profile-drawer candidate-application-detail" open={!!detail} onClose={() => { setDetail(null); setCandidateAttachments([]) }} width="min(1040px, 96vw)" title={candidateFromDetail ? `${candidateFromDetail.candidateName} · Candidate profile` : 'Candidate profile'}>{candidateFromDetail && <div className="candidate-360">
      ...
      <Card size="small" title="Hiring pipeline"><div className="candidate-hiring-timeline">{detail.applications.map(row => <article key={row.id}><i /><div><strong>{row.positionTitle}</strong><span>{row.applicationCode} · {row.currentStage}</span><small>Applied {formatApiDateTime(row.appliedAt)}</small></div><Space wrap><CandidateTrackingPreview applicationId={row.id} /><RecruitmentNegotiationAction applicationId={row.id} onSaved={refreshInterviewData} /><Tag color={row.atsScore == null ? 'orange' : row.atsScore >= 60 ? 'green' : 'red'}>{row.atsScore == null ? 'ATS pending' : `ATS ${row.atsScore.toFixed(1)}`}</Tag>{row.atsScore == null && canMoveApplication(row) && <Button size="small" icon={<RobotOutlined />} disabled={!row.resumeId || row.resumeAvailable === false} onClick={() => void scoreApplication(row.id).then(async () => { await load(); await refreshDetail() })}>Run ATS now</Button>}<Button size="small" type="primary" onClick={() => navigate(`/recruitment/hiring-pipeline?clientId=${row.clientId}&positionId=${row.positionId}&flow=candidates`)}>Open pipeline</Button></Space></article>)}</div></Card>
    ...
    {interviewDraft && <RecruitmentInterviewEditor mode="schedule" open applications={applications} positions={positions} panelUsers={panelUsers} standalone={Boolean(interviewDraft.standalone)} onCreateStandaloneApplication={createStandaloneInterviewApplication} interview={interviewDraft.interview} initialDecision={interviewDraft.initialDecision} initialApplicationId={interviewDraft.applicationId} initialStartNow={Boolean(interviewDraft.startNow)} onClose={() => setInterviewDraft(null)} onSaved={refreshInterviewData} />}
  ...
  return <div className="talent-workspace">
    {mode === 'candidates' && (new URLSearchParams(location.search).get('pool') === 'client' ? <>
      <RecruitmentRecordList emptyText={initialClientId ? "No candidate profiles for this client." : "Select a client to view its candidate profiles."} hiddenCardColumns={["candidateCode"]} rows={candidates.filter(row => initialClientId > 0 && row.clientId === initialClientId)} title={row => row.candidateName} subtitle={row => row.candidateCode} filters={[{ key: 'status', label: 'Status', value: row => row.profileStatus }, { key: 'source', label: 'Source', value: row => row.sourceType || '' }]} exportFileName="client-candidate-profiles" actions={row => <Button size="small" onClick={() => void openCandidate(row.id)}>View profile</Button>} columns={[
    ...
    </> : <RecruitmentGlobalTalentPool onViewCandidate={openCandidate} onChanged={load} activeTab={new URLSearchParams(location.search).get('pool') === 'matches' ? 'matches' : new URLSearchParams(location.search).get('pool') === 'selected' ? 'selected' : 'resumes'} />)}

    {mode === 'interviewQueue' && <section className="interview-operations-workspace">
      <PageHeaderPortal slot="recruitment-page-controls"><Space wrap><NewInterviewMenu onSelect={(startNow) => setInterviewDraft({ applicationId: 0, standalone: true, startNow })} /><Button type="primary" icon={<CalendarOutlined />} disabled={!selectedBatchApplications.length} onClick={() => setBatchInterviewOpen(true)}>Schedule selected ({selectedBatchApplications.length})</Button></Space></PageHeaderPortal>
      <RecruitmentRecordList rows={interviewReadyApplications} title={row => row.candidateName} subtitle={row => row.positionTitle} exportFileName="interview-queue"
        emptyText="No ATS-qualified candidates match these filters." selectionGroup={row => row.positionId}
        ...
        rowSelection={{ preserveSelectedRowKeys: true, selectedRowKeys: batchInterviewApplicationIds, onChange: keys => { const ids = keys.map(Number); if (new Set(interviewReadyApplications.filter(row => ids.includes(row.id)).map(row => row.positionId)).size > 1) { notify('Choose one job or select one candidate before selecting all.', 'warning'); return }; setBatchInterviewApplicationIds(ids) }, getCheckboxProps: row => ({ disabled: Boolean(selectedBatchPositionId && row.positionId !== selectedBatchPositionId) }) }}
        columns={[
    ...
    {mode === 'interviews' && <section className="interview-operations-workspace">
      ...
      {canScheduleInterviews && <PageHeaderPortal slot="recruitment-page-controls"><NewInterviewMenu onSelect={(startNow) => setInterviewDraft({ applicationId: 0, standalone: true, startNow })} /></PageHeaderPortal>}
    ...
    {mode === 'offers' && <>
      <PageHeaderPortal slot="recruitment-page-controls"><div className="talent-toolbar right">{session?.user.clientId == null && session?.user.roles.includes('super_admin') && <FinalOfferSettings clients={clients} initialClientId={initialClientId} reload={load} />}{!negotiationMode && <Button type="primary" icon={<PlusOutlined />} onClick={() => { setCandidateAttachments([]); setOfferDraft({ applicationId: 0, offeredCtc: 0, currency: 'INR', proposedJoiningDate: new Date(Date.now() + 30 * 86400000).toISOString().slice(0, 10), status: 'Draft' }) }}>Create offer</Button>}</div></PageHeaderPortal>
      {negotiationMode ? <Tabs className="negotiation-list-tabs" items={[
        { key: 'candidates', label: 'Candidate terms', children: <RecruitmentRecordList rows={applications} title={row => row.candidateName} subtitle={row => row.positionTitle} exportFileName="candidate-negotiation" filters={[{ key: 'job', label: 'Job', value: row => row.positionTitle }]} actions={row => <RecruitmentNegotiationAction applicationId={row.id} onSaved={refreshInterviewData} />} columns={[{ key: 'candidateName', label: 'Candidate' }, { key: 'positionTitle', label: 'Job' }, { key: 'currentStage', label: 'Stage' }]} /> },
        { key: 'offers', label: 'Offers & approvals', children: offerList },
      ]} /> : offerList}
    </>}
    ...
    {interviewDraft && <RecruitmentInterviewEditor mode="schedule" open applications={applications} positions={positions} panelUsers={panelUsers} standalone={Boolean(interviewDraft.standalone || interviewDraft.interview?.isStandalone)} initialStartNow={Boolean(interviewDraft.startNow)} onCreateStandaloneApplication={createStandaloneInterviewApplication} interview={interviewDraft.interview} initialDecision={interviewDraft.initialDecision} initialApplicationId={interviewDraft.applicationId} onClose={() => setInterviewDraft(null)} onSaved={refreshInterviewData} />}
