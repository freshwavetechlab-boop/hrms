            ...
            return <article key={row.id} className="candidate-application-card" tabIndex={0} role="button" onClick={() => void openCandidate(row.candidateId)} onKeyDown={event => { if (event.key === 'Enter') void openCandidate(row.candidateId) }}>
              ...
              <div className="candidate-card-actions" onClick={event => event.stopPropagation()}>
                ...
                {row.currentStage !== 'Joined' && <Select
                  ...
                  value={undefined}
                  options={[
                    { value: 'override', label: 'Override ATS', disabled: row.atsScore == null || rejectedApplication(row) || row.currentStage.startsWith('Offer') },
                ...
                />}
                <Space wrap><Button size="small" icon={<FileSearchOutlined />} onClick={() => void viewApplicationResume(row.id)}>Resume</Button>{canMoveApplication(row) && !row.autoRunAts && <Tooltip title={row.resumeId ? 'Calculate or refresh ATS evidence' : 'Add a resume before ATS scoring'}><Button size="small" disabled={!row.resumeId} icon={<RobotOutlined />} onClick={() => void scoreApplication(row.id).then(load)}>Run ATS</Button></Tooltip>}<Button size="small" type="primary" icon={<BranchesOutlined />} onClick={() => navigate(`/recruitment/hiring-pipeline?clientId=${row.clientId}&positionId=${row.positionId}&flow=candidates`)}>Pipeline</Button>{canDeleteRecruitmentData && <Popconfirm title="Delete application permanently?" description="This removes its ATS and pipeline data. If it is the candidate's only application, the candidate profile and stored resume file are also permanently deleted." okText="Delete" okButtonProps={{ danger: true }} onConfirm={async () => { const response = await deleteApplication(row.id); if (response.ok) await load() }}><Button size="small" danger icon={<DeleteOutlined />} aria-label={`Delete ${row.applicationCode}`} /></Popconfirm>}</Space>
              </div>
            </article>
