  ...
  return <div className="talent-workspace">
    ...
    {mode === 'interviews' && <section className="interview-operations-workspace">
      ...
      <DataTable rows={visibleInterviews} emptyText="No interviews match this status." exportFileName="recruitment-interviews" actions={row => <Space wrap>
        ...
        {row.canRecordDecision && <Button size="small" type="primary" onClick={() => setInterviewDraft({ applicationId: row.applicationId, interview: { ...row, status: 'Completed', result: 'Selected' } })}>Select</Button>}
        {row.canRecordDecision && <Button size="small" danger onClick={() => setInterviewDraft({ applicationId: row.applicationId, interview: { ...row, status: 'Completed', result: 'Rejected' } })}>Reject</Button>}
    ...
    <Drawer className="candidate-profile-drawer" open={!!detail} onClose={() => { setDetail(null); setCandidateAttachments([]) }} width="min(1180px, 96vw)" title={candidateFromDetail ? `${candidateFromDetail.candidateCode} - ${candidateFromDetail.candidateName}` : 'Talent profile'}>{candidateFromDetail && <div className="candidate-360">
      ...
      <Card size="small" title="Interview history"><DataTable rows={detail.interviews} actions={row => <Space wrap><Button size="small" icon={<FileSearchOutlined />} onClick={() => void viewApplicationResume(row.applicationId)}>Resume</Button>{!['Completed', 'Cancelled', 'No Show'].includes(row.status) && <Button size="small" onClick={() => setInterviewDraft({ applicationId: row.applicationId, interview: row })}>Update</Button>}{['Scheduled', 'Rescheduled'].includes(row.status) && <Button size="small" onClick={() => setInterviewDraft({ applicationId: row.applicationId, interview: { ...row, status: 'No Show', result: 'No Show' } })}>No show</Button>}{['Scheduled', 'Rescheduled'].includes(row.status) && <Button size="small" danger onClick={() => setInterviewDraft({ applicationId: row.applicationId, interview: { ...row, status: 'Cancelled', result: 'Pending' } })}>Cancel</Button>}<Button size="small" type="primary" onClick={() => setFeedbackInterview(row)}>{row.status === 'Completed' || (row.submittedPanelUserIds || []).length > 0 ? 'View feedback' : 'Panel feedback'}</Button>{canDeleteRecruitmentData && <Popconfirm title="Delete this interview?" description="Panel feedback and safe interview documents will also be removed." okText="Delete" okButtonProps={{ danger: true }} onConfirm={async () => { const response = await deleteInterview(row.id); if (response.ok) { await load(); await refreshDetail() } }}><Button size="small" danger icon={<DeleteOutlined />} aria-label={`Delete interview ${row.roundCode}`} /></Popconfirm>}</Space>} columns={[{ key: 'roundCode', label: 'Round' }, { key: 'interviewType', label: 'Type' }, { key: 'scheduledStart', label: 'Schedule', render: row => new Date(row.scheduledStart).toLocaleString('en-IN') }, { key: 'status', label: 'Status' }, { key: 'result', label: 'Result' }, { key: 'overallScore', label: 'Panel average' }]} /></Card>
