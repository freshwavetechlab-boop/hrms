...
import { getDropdowns, getRecruitmentAdminSetup, getWorkLocations, saveRecruitmentApprovalMapping, saveRecruitmentAssignmentRule, saveRecruitmentDocumentChecklist, saveRecruitmentMaster, saveRecruitmentPartner, saveRecruitmentSetting, saveRecruitmentSlaRule, saveRecruitmentTemplate } from '../services/settingsService'
...
function GridTable<T extends object>(p: { title: string; text: string; action: string; rows: T[]; columns: any[]; add: () => void; edit: (row: T) => void }) {
  return <>
    <Header title={p.title} text={p.text} action={p.action} onClick={p.add} />
    <DataTable rows={p.rows} columns={p.columns} actions={row => <Button size="small" type="primary" onClick={() => p.edit(row)}>Edit</Button>} />
  </>
}
function PartnerTable(p: { rows: RecruitmentPartner[]; type: 'Consultant' | 'Vendor'; open: (row?: RecruitmentPartner) => void }) {
  return <GridTable title={`${p.type} management`} text={`Client-wise ${p.type.toLowerCase()} setup and commission details.`} action={`Add ${p.type.toLowerCase()}`} rows={p.rows} add={() => p.open()} edit={p.open} columns={[{ key: 'clientName', label: 'Client' }, { key: 'code', label: 'Code' }, { key: 'name', label: p.type }, { key: 'contactPerson', label: 'Contact' }, { key: 'commissionValue', label: 'Commission' }, { key: 'status', label: 'Status' }]} />
}
