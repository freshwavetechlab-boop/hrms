...
const leaveTypePreviewRules: ImportPreviewRules = {
  ...
  enums: {
    ...
    'Attendance Action': ['Mark as leave', 'Mark as present'],
...
export default function LeaveTypesManager({ clientId, onMessage }: { clientId: number; onMessage: (message: string) => void }) {
  ...
  const downloadTemplate = () => {
    ...
    const rowValues = rows.length ? rows.map(row => [
      ...
      flag(row.encashUnusedLeaves), row.maxEncashmentLimit == null ? '' : String(row.maxEncashmentLimit), flag(row.allowNegativeLeaveBalance), flag(row.allowHalfDay), row.negativeBalanceHandling, row.attendanceAction,
...
function LeaveTypesTable(p: { rows: LeaveType[]; edit: (row: LeaveType) => void; toggle: (row: LeaveType) => void; remove: (row: LeaveType) => void }) {
  return <DataTable rows={p.rows} emptyText="No leave types configured." exportFileName="leave-types" columns={[
    ...
    { key: 'attendanceAction', label: 'After Approval', value: row => row.attendanceAction === 'Mark as present' ? 'Mark present' : 'Apply leave code' },
      ...
      <div className="leave-type-drawer-scroll">
        ...
        <section className="leave-type-form-section">
          ...
          <Row gutter={12}>
            ...
            <Col xs={24} md={12}><Form.Item label="After approval" extra={p.form.attendanceAction === 'Mark as present' ? 'Use for full-day attendance regularization. It marks Present and does not consume leave balance.' : 'The approved leave code appears in Attendance Review; paid/unpaid rules determine payable days.'}><SearchSelect value={p.form.attendanceAction} onChange={value => p.set('attendanceAction', value as LeaveType['attendanceAction'])} options={[{ value: 'Mark as leave', label: 'Apply leave code' }, { value: 'Mark as present', label: 'Mark present (regularization)' }]} /></Form.Item></Col>
