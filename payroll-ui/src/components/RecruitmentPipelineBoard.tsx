...
export default function RecruitmentPipelineBoard({ initialClientId = 0, clientScopeManaged = false, positionId: suppliedPositionId = 0, onOpenCandidate, onScheduleInterview, embedded = false, displayMode, onDisplayModeChange, clientPipelineCandidateCount, onCandidateCountChange, clientPipelineLanes = [] }: Props) {
  ...
  return <section className="orchestration-shell" data-testid="recruitment-hiring-pipeline">
    ...
    <Card size="small" className="candidate-pipeline-filters"><div className="candidate-pipeline-filter-grid">
      ...
      <div className="candidate-pipeline-filter-field"><span>Position / job posting {board?.pipelineVersionId && <Tag>Pipeline #{board.pipelineVersionId}</Tag>}</span><Select aria-label="Pipeline position or job posting" showSearch optionFilterProp="label" value={selectedTargetKey} placeholder="Select position or job posting" options={targetOptions} onChange={chooseTarget} /></div>
    ...
    {loading ? <Card loading data-testid="candidate-pipeline-loading" /> : !hasAssignedPipeline ? <Card className="candidate-pipeline-empty"><Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<div><strong>{positionId ? `Candidate pipeline not assigned${board?.positionTitle ? ` — ${board.positionTitle}` : ''}` : 'Select a position'}</strong><p>{positionId ? 'This position needs its own published pipeline assignment. The client-wide candidate total includes other positions.' : targetOptions.length ? 'Choose a position or published job posting above.' : 'No open positions or published job postings found for this client.'}</p></div>}><Space wrap>{positions.filter(row => row.id !== positionId && row.candidateCount > 0 && (!clientId || row.clientId === clientId)).slice(0, 3).map(row => <Button key={row.id} onClick={() => chooseTarget(`position:${row.id}`)}>View {row.positionTitle} ({row.candidateCount})</Button>)}<Button href={`/recruitment/hiring-pipeline?manage=1${clientId ? `&clientId=${clientId}` : ''}`}>Manage pipeline</Button><Button type="primary" href={`/recruitment/applications?upload=single${clientId ? `&clientId=${clientId}` : ''}${positionId ? `&positionId=${positionId}` : ''}`}>Add candidate resume</Button></Space></Empty></Card> : <>
      {!board && <div className="candidate-pipeline-summary"><Space wrap><Tag color="blue">Published template</Tag><strong>{templatePipeline?.pipelineName || 'Candidate pipeline'}</strong><Tag>Pipeline version #{templatePipeline?.pipelineVersionNumber || '-'}</Tag></Space></div>}
      <div className={`pipeline-results${viewMode === 'both' ? ' is-both' : ''}`}>
      ...
      {viewMode !== 'pipeline' && <Card size="small" className="pipeline-table-view" data-testid="pipeline-table-view"><DataTable fillHeight hideSearch
        rows={tableRows}
        getRowId={row => row.id}
        ...
        columns={[
          ...
          { key: 'interview', label: 'Interview score', width: '120px', value: row => row.card.interviewScore ?? '', render: row => row.card.interviewScore == null ? '-' : `${row.card.interviewScore.toFixed(1)} / 100` },
          ...
          { key: 'actions', label: 'Actions', width: '390px', sortable: false, filterable: false, render: row => <Space className="pipeline-table-actions" size={4} wrap>
            ...
            <RecruitmentNegotiationAction applicationId={row.card.applicationId} onSaved={() => loadBoard(positionId, postingId || undefined)} />
...
function CandidateCard({ lane, card, elapsedSinceLoad, transitions, recoverySelected, onRecoverySelected, onLoadTransitions, onOpen, onHistory, onSchedule, onDocuments, onDelete, onPause, onResume, onTransition }: {
  ...
  return <Card size="small" className="pipeline-card" data-testid={`pipeline-candidate-${card.applicationId}`}>
    {card.positionTitle && <div data-testid="candidate-job" style={{ whiteSpace: 'normal', overflowWrap: 'anywhere', marginBottom: 8 }}><strong>{card.positionTitle}</strong>{card.positionCode && <div>{card.positionCode}</div>}</div>}
    <div className={`pipeline-card-name ${onRecoverySelected ? 'has-selection' : ''}`}>{onRecoverySelected && <Checkbox aria-label={`Select ${card.candidateName} for return`} checked={recoverySelected} onChange={event => onRecoverySelected(event.target.checked)} />}<strong title={card.candidateName}>{card.candidateName}</strong>{card.atsScore == null ? <Tag>ATS pending</Tag> : <Tag color={card.atsScore >= 60 ? 'green' : 'orange'}>ATS {card.atsScore.toFixed(1)}</Tag>}{card.interviewScore != null && <Tag color={card.interviewScore >= 60 ? 'green' : 'orange'}>Interview {card.interviewScore.toFixed(1)}</Tag>}</div>
    ...
    <div className="pipeline-card-footer"><Tag data-testid="sla-clock" className={`sla-chip ${sla.className}`}>{sla.label}: {sla.clock}</Tag><Space size={4} wrap>
      ...
      <RecruitmentNegotiationAction applicationId={card.applicationId} onSaved={onLoadTransitions} />
