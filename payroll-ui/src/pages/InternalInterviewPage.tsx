    ...
    {view && <>
      ...
      {candidate && !view.consentAtUtc && !finished && <Card title="Before you join">
        ...
        <Space direction="vertical">
          ...
          {view.configuration.transcriptionEnabled && <Checkbox checked={transcriptConsent} onChange={e => setTranscriptConsent(e.target.checked)}>I consent to saving my written answers{view.speechEnabled ? ' and local speech transcription' : ''} for an AI-assisted draft and human review.</Checkbox>}
      ...
      {candidate && view.speechEnabled === true && view.status === 'Live' && view.transcriptionConsent && !answered && <InterviewVoiceControls client={client} question={lastQuestion || events.find(event => event.kind === 'introduction')} answerSeconds={view.configuration.answerSeconds} publishVoice={voicePublisher} microphoneId={devices.microphoneId} speakerId={devices.speakerId} onDraft={setText} onError={setError} />}
