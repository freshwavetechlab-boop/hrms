...
import { Alert, Button, Card, Checkbox, Form, Input, Result, Skeleton, Space, Tag, message } from 'antd'
...
export default function PublicCareersPage({ slug: suppliedSlug }: Props) {
  ...
  const begin = async () => {
    if (!email.trim() && !phone.trim()) return message.warning('Enter an email address or phone number.')
    if (!consent) return message.warning('Accept the candidate-data consent statement to continue.')
    setStarting(true)
    ...
    setStarting(false)
    if (!response.ok || !response.data) return message.error(response.error || 'Unable to start an application.')
  ...
  const submit = async () => {
    ...
    if (!saved.ok) { setSubmitting(false); return message.error(saved.error || 'Unable to save your application.') }
    const response = await submitPublicApplication(session.sessionToken)
    setSubmitting(false)
    if (!response.ok || !response.data) return message.error(response.error || 'Unable to submit your application.')
  ...
  const closed = Boolean(job.closesAtUtc && new Date(job.closesAtUtc).getTime() < Date.now())
  const unavailable = closed || !job.applicationForm
    ...
    <div className="public-job-layout"><Card className="public-job-content"><div className="public-job-copy"><h3>About the role</h3><p>{job.rolePurpose || job.summary}</p>{!!job.responsibilities.length && <><h3>What you will do</h3><ul>{job.responsibilities.map(item => <li key={item.id}>{item.responsibilityText}</li>)}</ul></>}{!!job.qualifications.length && <><h3>What we are looking for</h3><ul>{job.qualifications.map(item => <li key={item.id}>{item.qualificationName}{item.specialization ? ` · ${item.specialization}` : ''}{item.isMandatory ? ' (required)' : ''}</li>)}</ul></>}{!!job.skills.length && <><h3>Skills</h3><Space wrap>{job.skills.map(item => <Tag color={item.isRequired ? 'purple' : 'default'} key={item.id}>{item.skillName}{item.minimumYears ? ` · ${item.minimumYears}+ yrs` : ''}</Tag>)}</Space></>}</div></Card>
      <Card className="public-apply-card" id="public-application-form">
        {!session ? <><h2>Start your secure application</h2><p>No employee login is required. We use these details only to create a time-limited application session and prevent duplicate submissions.</p><Alert showIcon type="info" icon={<SafetyCertificateOutlined />} message="Documents are private and never exposed through direct storage links." />
          <Form layout="vertical" style={{ marginTop: 16 }}><Form.Item label="Email address"><Input type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="name@example.com" /></Form.Item><Form.Item label="Phone number"><Input type="tel" value={phone} onChange={event => setPhone(event.target.value)} placeholder="Mobile number" /></Form.Item><Checkbox checked={consent} onChange={event => setConsent(event.target.checked)}>I consent to the use of my information for this recruitment process and the configured retention period.</Checkbox><Button block size="large" type="primary" loading={starting} disabled={unavailable} onClick={() => void begin()} style={{ marginTop: 16 }}>{closed ? 'Applications closed' : !job.applicationForm ? 'Application form unavailable' : 'Continue to application'}</Button></Form>
