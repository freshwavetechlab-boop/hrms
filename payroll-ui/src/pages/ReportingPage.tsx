...
function ReportingWorkspace({ activeReport }: { activeReport: ReportDefinition }) {
  ...
  return <section className={`reporting-page${isEmployeeMaster ? ' employee-master-report' : ''}`}>
    ...
    {activeReport.code ? <div className="card report-result">{reportError
      ? <div className="report-load-state"><Alert showIcon type="error" message="Report could not be loaded" description={reportError} action={<Button onClick={() => setReportReloadKey(value => value + 1)}>Retry</Button>} /></div>
      : reportLoading
        ? <div className="report-load-state" role="status"><Spin /><span>Loading report data...</span></div>
        : <DataTable<ReportRow>
          rows={result.rows}
