...
export default function SettingsPage({ tab, onMessage }: { tab: SettingsTab; onMessage: (message: string) => void }) {
  ...
  const saveAll = async (event: FormEvent) => { event.preventDefault(); setSaving(true); await saveOrganization(org); await saveSetup({ ...setup, statutory: { ...setup.statutory, ptNumber: org.professionalTaxNumber || setup.statutory.ptNumber } }); notify('Settings saved.'); setSaving(false) }
  const saveClient = async () => { if (!client.name.trim()) return; await persistClient(client); setClient(client0); await load(); notify('Client saved.') }
  ...
  const saveLocation = async () => {
    if (!location.clientId) return notify('Client select karo.', 'error')
    if (!location.name.trim()) return notify('Location name required hai.', 'error')
    const response = await saveWorkLocation(location)
    notify(response.ok ? 'Work location saved.' : response.error || 'Location fields check karo.', response.ok ? 'success' : 'error')
  ...
  return <form onSubmit={saveAll}>
    ...
    {tab !== 'Salary Components' && <div className="actions"><p>Structures are client-wise. Components are global.</p><button disabled={saving || tab === 'Clients'}>{saving ? 'Saving...' : 'Save settings'}</button></div>}
