...
export default function SettingsPage({ tab, onMessage }: { tab: SettingsTab; onMessage: (message: string) => void }) {
  ...
  return <form onSubmit={saveAll}>
    ...
    {tab === 'Work Locations' && <Card t="Work locations"><div className="grid"><F l="Location name"><input value={location.name} onChange={event => setLocation({ ...location, name: event.target.value })} placeholder="Head Office / WFH - Employee Name" /></F><F l="State"><Sel v={location.state} set={value => setLocation({ ...location, state: value, city: '' })} a={stateOptions} /></F><F l="Address" w><input value={location.address} onChange={event => setLocation({ ...location, address: event.target.value })} /></F><F l="City"><Sel v={location.city} set={value => setLocation({ ...location, city: value })} a={cityOptions(location.state)} /></F><F l="PIN code"><input value={location.postalCode} onChange={event => setLocation({ ...location, postalCode: event.target.value.replace(/\D/g, '').slice(0, 6) })} /></F><Chk l="Primary work location" v={location.isPrimary} set={value => setLocation({ ...location, isPrimary: value })} /><Chk l="Active" v={location.isActive} set={value => setLocation({ ...location, isActive: value })} /><button type="button" onClick={saveLocation}>{location.id ? 'Update location' : 'Add location'}</button></div><DataTable rows={locations} onEdit={setLocation} columns={[{ key: 'name', label: 'Location' }, { key: 'city', label: 'City' }, { key: 'state', label: 'State' }, { key: 'postalCode', label: 'PIN' }, { key: 'isPrimary', label: 'Primary', render: item => item.isPrimary ? 'Yes' : 'No' }]} /></Card>}
