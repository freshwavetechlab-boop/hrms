...
export default function SettingsPage({ tab, onMessage }: { tab: SettingsTab; onMessage: (message: string) => void }) {
  ...
  const clientScopedAdmin = Boolean(session?.user.clientId && !session.user.permissions.includes('settings.manage'))
  ...
  const load = async () => {
    if (clientScopedAdmin) {
      const [rawSetup, clientRows] = await Promise.all([getSetup(settingsSetup0), getClients()])
      setSetup({ ...settingsSetup0, ...rawSetup, tax: { ...setup0.tax, ...rawSetup.tax, clientSettings: rawSetup.tax?.clientSettings ?? [] }, schedule: { ...setup0.schedule, ...rawSetup.schedule }, statutory: { ...setup0.statutory, ...rawSetup.statutory }, salaryComponents: (rawSetup.salaryComponents ?? []).map(normalizeComponentForUi), salaryStructures: rawSetup.salaryStructures ?? [], payslipTemplates: rawSetup.payslipTemplates ?? [] })
      setClients(clientRows)
      const scopedId = String(session?.user.clientId || clientRows[0]?.id || '')
      setStructure(current => ({ ...current, clientId: scopedId }))
      setPayslip(current => ({ ...current, clientId: scopedId }))
      return
    }
  ...
  const uploadClientLogo = (file: File) => {
    if (!['image/png', 'image/jpeg', 'image/webp'].includes(file.type)) {
      notify('Client logo must be a PNG, JPG or WebP image.', 'error')
      return
    }
    if (file.size > 1024 * 1024) {
      notify('Client logo must be 1 MB or smaller.', 'error')
      return
    }
    const reader = new FileReader()
    reader.onload = () => setClient(current => ({ ...current, logoDataUrl: String(reader.result || '') }))
    reader.readAsDataURL(file)
  }
  ...
  const saveStructure = async () => {
    ...
    setSetup(nextSetup)
    setStructure(clientScopedAdmin ? { ...structure0, clientId: String(session?.user.clientId || '') } : structure0)
  ...
  const savePayslip = async () => {
    ...
    setSetup(nextSetup)
    setPayslip(clientScopedAdmin ? { ...payslip0, clientId: String(session?.user.clientId || '') } : payslip0)
  ...
  const renderClientBilling = () => {
    return <AntCard size="small" className="settings-panel settings-table-panel" aria-label="Advanced costing rules">
  ...
  return <form onSubmit={saveAll}>
    {tab === 'Organization' && <AntCard size="small" className="settings-panel organization-settings-page" aria-label="Organization configuration"><PageTabs items={organizationTabs} value={organizationTab} onChange={setOrganizationTab} label="Organization sections" />{renderOrganizationBody()}</AntCard>}
    {tab === 'Clients' && <>
      <AntCard size="small" className="settings-panel settings-table-panel" aria-label="Client master">
        ...
        <DataTable rows={clients.filter(item => item.isActive)} actions={row => <Space size={6}><Button size="small" type="primary" onClick={() => { setClient(row); setClientDrawerOpen(true) }}>Edit</Button><Button size="small" danger onClick={() => void deleteClient(row)}>Delete</Button></Space>} columns={[{ key: 'name', label: 'Client', render: item => <span className="client-master-name">{item.logoDataUrl ? <img src={item.logoDataUrl} alt="" /> : <b>{item.name.slice(0, 1).toUpperCase()}</b>}<span>{item.name}</span></span> }, { key: 'code', label: 'Code' }, { key: 'contactPerson', label: 'Contact' }, { key: 'email', label: 'Email' }, { key: 'isActive', label: 'Status', render: item => item.isActive ? 'Active' : 'Inactive' }]} />
      </AntCard>
    ...
    {tab === 'AI Integration' && <AiIntegrationSettings />}
    ...
    {!['Clients', 'Client Billing Configuration', 'Travel & Expense Policies', 'Notifications', 'AI Integration', 'Scheduled Jobs', 'Attachments', 'Salary Components', 'Salary Templates', 'Payslip Templates', 'Work Locations', 'Dropdown Masters', 'ESS Settings'].includes(tab) && <div className="actions"><p>Structures are client-wise. Components are global.</p><button disabled={saving}>{saving ? 'Saving...' : 'Save settings'}</button></div>}
