...
export default function SettingsPage({ tab, onMessage }: { tab: SettingsTab; onMessage: (message: string) => void }) {
  ...
  return <form onSubmit={saveAll}>
    ...
    {!['Salary Components', 'Salary Templates', 'Payslip Templates', 'Work Locations'].includes(tab) && <div className="actions"><p>Structures are client-wise. Components are global.</p><button disabled={saving || tab === 'Clients'}>{saving ? 'Saving...' : 'Save settings'}</button></div>}
