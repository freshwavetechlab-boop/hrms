...
export const deleteTravelExpenseConfiguration = (kind: string, id: number, clientId?: number) => deleteJson(`/api/travel-expense/${kind}/${id}${clientId ? `?clientId=${clientId}` : ''}`, null, { successMessage: kind === 'client-setting' ? 'Client enablement reset.' : 'Travel & Expense configuration deleted.' })
