...
export const updateOfferStatus = async (id: number, status: string, remarks = '') => { const result = await postJson(`/api/recruitment/offers/${id}/status`, { status, remarks }, null as RecruitmentOffer | null, { successMessage: 'Offer status updated.' })
; if (result.ok && result.data?.pipelineTransitionMessage) toast.info(result.data.pipelineTransitionMessage); return result }
