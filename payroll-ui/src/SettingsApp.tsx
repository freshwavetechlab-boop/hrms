...
const moduleCodes = ['Settings', 'Employees', 'Payroll', 'Security', 'LeaveAttendance', 'Reports', 'Workflows'] as const
...
type PayrollTab = 'Attendance Review' | 'Pay Run'
...
export default function SettingsApp() {
  ...
  const [payrollTab, setPayrollTab] = useState<PayrollTab>((localStorage.getItem('payroll.payrollTab') as PayrollTab | null) === 'Pay Run' ? 'Pay Run' : 'Attendance Review')
  ...
  const pageDescription = mainModule === 'Payroll' ? isPayHistory ? 'All client-wise draft, approved and paid payroll runs.' : payrollTab === 'Attendance Review' ? 'Review client attendance, resolve discrepancies, then run payroll.' : 'Draft, review and approve monthly payroll.' : mainModule === 'Security' ? 'Manage identities, roles, permissions and audit evidence.' : mainModule === 'Employees' ? 'Maintain client-linked employee records, salary profiles and payment details.' : mainModule === 'LeaveAttendance' ? 'Setup leave types, holidays, attendance preferences and opening balances.' : settingsMessage
  ...
  const renderPage = () => {
    ...
    if (mainModule === 'Payroll') return isPayHistory ? <PayHistoryPage /> : payrollTab === 'Attendance Review' ? <PayrollAttendancePage /> : <PayrollPage />
