...
CREATE TABLE IF NOT EXISTS ResourceStates (
    ...
    public async Task<bool> IsActionRuleActiveAsync(string activityCode){await using var db=Db();await db.OpenAsync();return await db.ExecuteScalarAsync<int>("SELECT COUNT(*) FROM workflow_action_rules WHERE ActivityCode=@ActivityCode AND IsActive=TRUE",new{ActivityCode=activityCode.Trim()})>0;}
    ...
    public async Task<WorkflowApproverPreview> GetApproverPreviewAsync(string? approverType,int? clientId,int? approverUserId)
    {
        var type=(approverType??"").Trim();
        if(type.Equals("Specific User",StringComparison.OrdinalIgnoreCase))
        {
            if(approverUserId.GetValueOrDefault()<=0)return new WorkflowApproverPreview{ApproverType="Specific User",Message="Select an active specific user to preview this stage."};
            await using var db=Db();await db.OpenAsync();
            var approver=await db.QueryFirstOrDefaultAsync<WorkflowApprover>("SELECT u.Id,u.DisplayName,u.Email,u.ClientId,COALESCE(c.Name,'All clients') ClientName FROM authusers u LEFT JOIN clients c ON c.Id=u.ClientId WHERE u.Id=@Id AND u.IsActive=TRUE",new{Id=approverUserId});
            return approver is null
                ? new WorkflowApproverPreview{ApproverType="Specific User",Message="The selected specific user is missing or inactive; this stage cannot resolve."}
                : new WorkflowApproverPreview{ApproverType="Specific User",ResolutionStatus="Resolved",Approver=approver,Message=$"Resolved to active user {approver.DisplayName} ({approver.Email})."};
        }
        if(type.Equals("HR Manager",StringComparison.OrdinalIgnoreCase))
        {
            if(clientId.GetValueOrDefault()<=0)return new WorkflowApproverPreview{ApproverType="HR Manager",ResolutionStatus="Dynamic",IsDynamic=true,Message="The HR Manager will be identified from the requestor's client when the request is submitted."};
            await using var db=Db();await db.OpenAsync();
            var approver=await db.QueryFirstOrDefaultAsync<WorkflowApprover>(@"SELECT u.Id,u.DisplayName,u.Email,u.ClientId,COALESCE(c.Name,'All clients') ClientName
FROM authusers u
...
WHERE u.IsActive=TRUE
  ...
  AND EXISTS (
      ...
      WHERE ur.UserId=u.Id AND r.Code='hr_manager'
  )
ORDER BY u.ClientId IS NULL,u.Id
LIMIT 1",new{ClientId=clientId});
            return approver is null
                ? new WorkflowApproverPreview{ApproverType="HR Manager",Message="No active client-specific HR Manager is configured for this client, and no active global HR Manager fallback is available."}
                : new WorkflowApproverPreview{ApproverType="HR Manager",ResolutionStatus="Resolved",Approver=approver,Message=$"Resolved to active user {approver.DisplayName} ({approver.Email})."};
        }
        if(type.Equals("Reporting Manager",StringComparison.OrdinalIgnoreCase))return new WorkflowApproverPreview{ApproverType="Reporting Manager",ResolutionStatus="Dynamic",IsDynamic=true,Message="The employee's reporting manager will be identified when the request is submitted."};
        if(type.Equals("Department Head",StringComparison.OrdinalIgnoreCase))return new WorkflowApproverPreview{ApproverType="Department Head",ResolutionStatus="Dynamic",IsDynamic=true,Message="The employee's assigned department head will be identified when the request is submitted."};
        return new WorkflowApproverPreview{ApproverType=type,Message=string.IsNullOrWhiteSpace(type)?"Approver type is required.":$"Approver type '{type}' is not supported."};
    }
...
SELECT Id FROM workflow_action_rules WHERE ActivityCode=@ActivityCode AND HttpMethod=@HttpMethod AND PathPattern=@PathPattern;",data);r.Id=id;}else await db.ExecuteAsync(@"UPDATE workflow_action_rules SET ActivityCode=@ActivityCode,HttpMethod=@HttpMethod,PathPattern=@PathPattern,ResourceType=@ResourceType,ResourceIdSource=@ResourceIdSource,ResourceIdRouteKey=@ResourceIdRouteKey,ClientIdSource=@ClientIdSource,ClientIdSql=@ClientIdSql,ClientLookupTable=@ClientLookupTable,ClientLookupKeyColumn=@ClientLookupKeyColumn,ClientLookupClientColumn=@ClientLookupClientColumn,WorkflowId=@WorkflowId,TriggerMode=@TriggerMode,IsActive=@IsActive WHERE Id=@Id",data);return await db.QuerySingleAsync<WorkflowActionRule>("SELECT * FROM workflow_action_rules WHERE Id=@Id",new{r.Id});}
    public async Task<(WorkflowMaster? Row,string Error)> SaveAsync(SaveWorkflowRequest r)
    {
        await using var db=Db();await db.OpenAsync();await using var tx=await db.BeginTransactionAsync();var id=r.Id;var preserveStages=false;
        if(id!=0)
        {
            // Save and Start take this lock first so a start either commits against the old stages
            // before the pending-edit guard runs, or reads the replacement stages after this save.
            var existing=await db.QueryFirstOrDefaultAsync<WorkflowMaster>("SELECT * FROM workflowmasters WHERE Id=@Id FOR UPDATE",new{Id=id},tx);
            var pendingInstanceId=await db.ExecuteScalarAsync<long?>("SELECT Id FROM workflowinstances WHERE WorkflowId=@Id AND Status='Pending' ORDER BY Id LIMIT 1 FOR UPDATE",new{Id=id},tx);
            if(pendingInstanceId is not null)
            {
                var current=(await db.QueryAsync<WorkflowStage>("SELECT * FROM workflowstages WHERE WorkflowId=@Id ORDER BY StageOrder,Id",new{Id=id},tx)).ToList();
                var requested=r.Stages.OrderBy(x=>x.StageOrder).ToList();
                var sameIdentity=existing is not null&&existing.ClientId==r.ClientId&&string.Equals(existing.Code.Trim(),r.Code.Trim(),StringComparison.OrdinalIgnoreCase)&&string.Equals(existing.ResourceType.Trim(),r.ResourceType.Trim(),StringComparison.OrdinalIgnoreCase);
                var same=sameIdentity&&current.Count==requested.Count&&current.Zip(requested).All(pair=>pair.First.StageOrder==pair.Second.StageOrder&&string.Equals(pair.First.Name.Trim(),pair.Second.Name.Trim(),StringComparison.Ordinal)&&string.Equals(pair.First.ApproverType.Trim(),pair.Second.ApproverType.Trim(),StringComparison.OrdinalIgnoreCase)&&pair.First.ApproverUserId==pair.Second.ApproverUserId);
                if(!same)return(null,"This workflow has pending approvals. Complete or cancel them before changing approval stages or assignees.");
                preserveStages=true;
            }
        }
        if(id==0)id=(int)await db.ExecuteScalarAsync<long>("INSERT INTO workflowmasters (ClientId,Code,Name,ResourceType,IsActive) VALUES (@ClientId,@Code,@Name,@ResourceType,@IsActive);SELECT LAST_INSERT_ID();",r,tx);else await db.ExecuteAsync("UPDATE workflowmasters SET ClientId=@ClientId,Code=@Code,Name=@Name,ResourceType=@ResourceType,IsActive=@IsActive WHERE Id=@Id",r,tx);
        if(!preserveStages)
        {
            await db.ExecuteAsync("DELETE FROM workflowstages WHERE WorkflowId=@Id",new{Id=id},tx);
            foreach(var s in r.Stages.OrderBy(x=>x.StageOrder))await db.ExecuteAsync("INSERT INTO workflowstages (WorkflowId,StageOrder,Name,ApproverType,ApproverUserId) VALUES (@WorkflowId,@StageOrder,@Name,@ApproverType,@ApproverUserId)",new{WorkflowId=id,s.StageOrder,s.Name,s.ApproverType,s.ApproverUserId},tx);
        }
        await tx.CommitAsync();return((await GetAsync()).First(x=>x.Id==id),"");
    }
    ...
    public async Task<IEnumerable<WorkflowRequestProgress>> RequesterProgressAsync(int requestorUserId,long? instanceId=null){await using var db=Db();await db.OpenAsync();return await db.QueryAsync<WorkflowRequestProgress>(@"SELECT i.Id AS InstanceId,COALESCE(m.Name,'Workflow') AS WorkflowName,i.ResourceType,i.ResourceId,i.Status,COALESCE(s.Name,'') AS CurrentStageName,COALESCE(s.ApproverType,'') AS CurrentApproverType,COALESCE(approver.DisplayName,'') AS PendingApproverName,t.CreatedAt AS PendingSince,i.CreatedAt,i.CompletedAt
FROM workflowinstances i
JOIN workflowmasters m ON m.Id=i.WorkflowId
LEFT JOIN workflowtasks t ON t.Id=(SELECT pending.Id FROM workflowtasks pending WHERE pending.InstanceId=i.Id AND pending.Status='Pending' ORDER BY pending.Id DESC LIMIT 1)
LEFT JOIN workflowstages s ON s.Id=t.StageId
LEFT JOIN authusers approver ON approver.Id=t.ApproverUserId
WHERE i.RequestorUserId=@RequestorUserId AND (@InstanceId IS NULL OR i.Id=@InstanceId)
ORDER BY i.CreatedAt DESC,i.Id DESC
LIMIT 250",new{RequestorUserId=requestorUserId,InstanceId=instanceId});}
    public async Task<IEnumerable<WorkflowTask>> ActionedAsync(int userId,bool all=false){await using var db=Db();await db.OpenAsync();return await db.QueryAsync<WorkflowTask>(@"SELECT t.Id,t.InstanceId,t.StageId,s.Name AS StageName,m.Code ActivityCode,m.ClientId,i.ResourceType,i.ResourceId,i.PayloadJson,t.ApproverUserId,COALESCE(approver.DisplayName,'') ApproverName,COALESCE(actor.DisplayName,approver.DisplayName,'') ActorName,COALESCE(NULLIF(t.Status,''),h.Action,'Actioned') Status,COALESCE(NULLIF(t.Comment,''),h.Comment,'') Comment,t.CreatedAt,COALESCE(t.ActionedAt,h.CreatedAt) ActionedAt
...
LIMIT 250",new{UserId=userId,All=all});}
    ...
    public async Task<WorkflowInstance?> GetInstanceForTaskAsync(long taskId){await using var db=Db();await db.OpenAsync();return await db.QueryFirstOrDefaultAsync<WorkflowInstance>("SELECT i.*,m.Code ActivityCode,m.ClientId FROM workflowinstances i JOIN workflowtasks t ON t.InstanceId=i.Id JOIN workflowmasters m ON m.Id=i.WorkflowId WHERE t.Id=@TaskId",new{TaskId=taskId});}
    public async Task<bool> CanReviewTaskAsync(long taskId,int userId){await using var db=Db();await db.OpenAsync();return await db.ExecuteScalarAsync<int>("SELECT COUNT(*) FROM workflowtasks WHERE Id=@TaskId AND ApproverUserId=@UserId",new{TaskId=taskId,UserId=userId})>0;}
    ...
    private static async Task<int?> ResolveAsync(MySqlConnection db,WorkflowStage s,int requestor,MySqlTransaction? tx=null){if(s.ApproverType=="Specific User")return await db.ExecuteScalarAsync<int?>("SELECT Id FROM authusers WHERE Id=@Id AND IsActive=TRUE",new{Id=s.ApproverUserId},tx);if(s.ApproverType=="HR Manager")return await db.ExecuteScalarAsync<int?>("SELECT u.Id FROM authusers u JOIN authuserroles ur ON ur.UserId=u.Id JOIN authroles r ON r.Id=ur.RoleId JOIN authusers requester ON requester.Id=@Requestor LEFT JOIN employees e ON e.Id=requester.EmployeeId WHERE r.Code='hr_manager' AND u.IsActive=TRUE AND (u.ClientId=e.ClientId OR u.ClientId IS NULL) ORDER BY u.ClientId IS NULL,u.Id LIMIT 1",new{Requestor=requestor},tx);if(s.ApproverType=="Reporting Manager")return await db.ExecuteScalarAsync<int?>(@"SELECT u.Id
...
ON DUPLICATE KEY UPDATE CurrentState=@State, WorkflowInstanceId=@WorkflowInstanceId, ModifiedBy=@Actor, ModifiedOn=CURRENT_TIMESTAMP",new{ResourceType=resourceType,ResourceId=resourceId,State=state,WorkflowInstanceId=workflowInstanceId,Actor=actor},tx);
