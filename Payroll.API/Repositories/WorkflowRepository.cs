...
LIMIT 250",new{UserId=userId,All=all});}
    ...
    private static async Task<int?> ResolveAsync(MySqlConnection db,WorkflowStage s,int requestor){if(s.ApproverType=="Specific User")return await db.ExecuteScalarAsync<int?>("SELECT Id FROM authusers WHERE Id=@Id AND IsActive=TRUE",new{Id=s.ApproverUserId});if(s.ApproverType=="HR Manager")return await db.ExecuteScalarAsync<int?>("SELECT u.Id FROM authusers u JOIN authuserroles ur ON ur.UserId=u.Id JOIN authroles r ON r.Id=ur.RoleId JOIN authusers requester ON requester.Id=@Requestor LEFT JOIN employees e ON e.Id=requester.EmployeeId WHERE r.Code='hr_manager' AND u.IsActive=TRUE AND (u.ClientId=e.ClientId OR u.ClientId IS NULL) ORDER BY u.ClientId IS NULL,u.Id LIMIT 1",new{Requestor=requestor});if(s.ApproverType=="Reporting Manager")return await db.ExecuteScalarAsync<int?>(@"SELECT u.Id
