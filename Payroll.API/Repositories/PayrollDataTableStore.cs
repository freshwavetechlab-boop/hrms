...
CREATE TABLE IF NOT EXISTS salarycomponents (
    ...
    ComponentRole VARCHAR(80) NOT NULL DEFAULT 'Regular Earning',
    StatutoryType VARCHAR(80) NOT NULL DEFAULT 'None',
...
CREATE TABLE IF NOT EXISTS payrunemployeelines (
    ...
    ComponentRole VARCHAR(80) NOT NULL DEFAULT '',
    StatutoryType VARCHAR(80) NOT NULL DEFAULT 'None',
    ...
    UNIQUE KEY UX_PayRunEmployeeLines_Row_Component (PayRunEmployeeId, ComponentCode, SortOrder)
        ...
        await EnsureColumnAsync(connection, "payrunemployeelines", "ComponentRole", "VARCHAR(80) NOT NULL DEFAULT '' AFTER Category");
...
LEFT JOIN (
    ...
    public static async Task SyncPayRunEmployeeLinesAsync(MySqlConnection connection, MySqlTransaction? transaction, PayRunEmployee row)
    {
        ...
        {
            await connection.ExecuteAsync(@"INSERT INTO payrunemployeelines (PayRunEmployeeId,PayRunId,EmployeeId,ComponentCode,Name,Category,ComponentRole,StatutoryType,MonthlyAmount,Amount,ProRata,SortOrder)
VALUES (@PayRunEmployeeId,@PayRunId,@EmployeeId,@ComponentCode,@Name,@Category,@ComponentRole,@StatutoryType,@MonthlyAmount,@Amount,@ProRata,@SortOrder)", new { PayRunEmployeeId = row.Id, row.PayRunId, row.EmployeeId, ComponentCode = line.Id, line.Name, line.Category, ComponentRole = line.ComponentRole, StatutoryType = string.IsNullOrWhiteSpace(line.StatutoryType) ? "None" : line.StatutoryType, line.MonthlyAmount, line.Amount, line.ProRata, SortOrder = order++ }, transaction);
        }
    }
...
WHERE table_schema = DATABASE() AND table_name = @TableName AND column_name = @ColumnName", new { TableName = tableName, ColumnName = columnName });
    ...
    private static async Task SaveComponentsAsync(MySqlConnection connection, List<ComponentDto> rows)
    {
        ...
        foreach (var row in rows.Where(x => x.Id > 0))
            await connection.ExecuteAsync(@"INSERT INTO salarycomponents (Id,Code,ComponentType,ComponentRole,StatutoryType,Category,Name,PayType,CalculationType,ValueText,Formula,BaseComponent,Taxable,Ctc,ProRata,Fbp,RestrictFbp,Epf,Esi,Recurring,Scheduled,InvestmentType,CorrectionOf,Active,Priority)
VALUES (@Id,@Code,@ComponentType,@ComponentRole,@StatutoryType,@Category,@Name,@PayType,@CalculationType,@Value,@Formula,@BaseComponent,@Taxable,@Ctc,@ProRata,@Fbp,@RestrictFbp,@Epf,@Esi,@Recurring,@Scheduled,@InvestmentType,@CorrectionOf,@Active,@PriorityNumber)
ON DUPLICATE KEY UPDATE Code=@Code,ComponentType=@ComponentType,ComponentRole=@ComponentRole,StatutoryType=@StatutoryType,Category=@Category,Name=@Name,PayType=@PayType,CalculationType=@CalculationType,ValueText=@Value,Formula=@Formula,BaseComponent=@BaseComponent,Taxable=@Taxable,Ctc=@Ctc,ProRata=@ProRata,Fbp=@Fbp,RestrictFbp=@RestrictFbp,Epf=@Epf,Esi=@Esi,Recurring=@Recurring,Scheduled=@Scheduled,InvestmentType=@InvestmentType,CorrectionOf=@CorrectionOf,Active=@Active,Priority=@PriorityNumber", new { row.Id, Code = Clean(row.Code).ToUpperInvariant(), row.ComponentType, ComponentRole = Clean(row.ComponentRole, DefaultComponentRole(row.Category)), StatutoryType = Clean(row.StatutoryType, "None"), row.Category, row.Name, row.PayType, row.CalculationType, row.Value, row.Formula, row.BaseComponent, row.Taxable, row.Ctc, row.ProRata, row.Fbp, row.RestrictFbp, row.Epf, row.Esi, row.Recurring, row.Scheduled, row.InvestmentType, row.CorrectionOf, row.Active, PriorityNumber = ToInt(row.Priority, 999) });
    }
...
ON DUPLICATE KEY UPDATE BankName=@BankName,BankAccountNo=@BankAccountNo,IfscCode=@IfscCode,PaymentMode=@PaymentMode", new { EmployeeId = employeeId, payment.BankName, payment.BankAccountNo, payment.IfscCode, payment.PaymentMode });
    ...
    private static string DefaultComponentRole(string category) => category.Equals("Deduction", StringComparison.OrdinalIgnoreCase) ? "Regular Deduction" : category.Equals("Reimbursement", StringComparison.OrdinalIgnoreCase) ? "Reimbursement" : category.Equals("Benefit", StringComparison.OrdinalIgnoreCase) ? "Employer Contribution" : category.Equals("Correction", StringComparison.OrdinalIgnoreCase) ? "Arrear / Correction" : "Regular Earning";
    ...
    private static async Task<bool> TableExistsAsync(MySqlConnection connection, string tableName) =>
        await connection.ExecuteScalarAsync<int>("SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=@tableName", new { tableName }) > 0;
