...
export function validateLocalSimpleCountPlan(ast, contract, parameters = []) {
  ...
  if (!sources.length || sources.length > allowed.size || lower(sources[0].table) !== record
    || sources.some(source => !allowed.has(lower(source.table)) || source.expr || source.using)) fail(`${aggregate ? 'aggregate' : 'count'} only the requested record table and required host relationships. Start FROM ${contract.recordTable}; add only joins needed for the requested grouping, filters or client scope.`)
  ...
  const column = expr => {
    ...
    // MySQL quoted qualifiers are identifier nodes, while bare qualifiers are
    // strings. Accept only the parser's exact quoted-identifier representation.
    const qualifier = typeof expr.table === 'string' ? expr.table
      : expr.table?.type === 'backticks_quote_string' && name(expr.table.value) ? expr.table.value : null
    if (expr.table && !qualifier) return null
    const table = qualifier ? aliases.get(lower(qualifier)) : sources.length === 1 ? record : null
  ...
  for (const source of sources.slice(1)) {
    ...
    if (!needed.has(table) || !relationship || source.on?.type !== 'binary_expr' || source.on.operator !== '=') fail(needed.size === 1
      ? `an unrequested or fanout-producing join was added. Remove every JOIN: this metric uses only ${contract.recordTable}; the backend applies its direct client scope.`
      : 'an unrequested or fanout-producing join was added. Join only tables required by the host grouping, filters or scope using their exact relationships.')
    ...
    const scopedInterviewParent = record === 'recruitment_interviews' && table === 'recruitment_candidate_applications'
      && (contract.scopeTables || []).some(scope => lower(scope) === table)
    if (scopedInterviewParent) {
      if (!same(relationship.from, { table: record, column: 'applicationid' })
        || !['INNER JOIN', 'JOIN'].includes(source.join)) fail('use INNER JOIN on the application parent so client scope restricts the requested interviews.')
    } else if (relationship.requiredJoinType === 'INNER JOIN') {
