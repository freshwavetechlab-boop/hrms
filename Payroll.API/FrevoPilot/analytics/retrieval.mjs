...
export function selectRelevantCatalog(question, catalog = []) {
  const ranked = catalog.map(table => ({ ...table, score: scoreCatalogTable(question, table) }))
    .sort((left, right) => right.score - left.score)
  const selected = ranked.slice(0, 12)
  if (selected.some(table => String(table.name).toLowerCase() === 'clients')) return selected

  // A client-owned record table is not a complete planning schema without its
  // client parent. Domain/stage matches can otherwise push clients below the
  // top twelve and prevent the host from recognizing an exact scoped metric.
  // Retain only an existing authorized table and a declared, exposed key pair.
  const hasColumn = (table, name) => (table?.columns || []).some(column =>
    !column.sensitive && String(column.name).toLowerCase() === name.toLowerCase())
  const clientTable = ranked.find(table => String(table.name).toLowerCase() === 'clients')
  if (!hasColumn(clientTable, 'Id')) return selected
  const clientDependency = selected.find(child => relationshipRules.some(rule => {
    const match = /^([A-Za-z0-9_]+)\.([A-Za-z0-9_]+) -> clients\.Id$/.exec(rule)
    return match && String(child.name).toLowerCase() === match[1].toLowerCase() && hasColumn(child, match[2])
  }))
  if (clientDependency) {
    const replaceIndex = selected.length - (selected.at(-1) === clientDependency ? 2 : 1)
    selected.splice(replaceIndex, 1)
    selected.push(clientTable)
  }
  return selected
}
