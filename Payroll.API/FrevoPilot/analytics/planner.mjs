...
export function enforceSemanticGuards(plan, question, catalog, sourceDatabase) {
  // Recognized local metrics have an authoritative host contract. The runtime
  // validates its SQL, measure, grouping, filters and scope before any query;
  // text/alias heuristics must not rewrite those exact requested metrics.
  if (String(plan?.providerCode || '').toLowerCase() === 'localopenaicompatible'
    && plan.simpleCountContract?.version === 'simple-count-v1') return plan
...
function hostPlanMetadata(response) {
  // Receipts and reuse decisions belong to the authenticated host. Generated
  // payload fields cannot promote a plan or suppress a fresh AI summary.
  if (String(response.providerCode || '').toLowerCase() !== 'localopenaicompatible'
    || typeof response.planReceipt !== 'string' || !response.planReceipt.trim()) return {}
  return {
    planReceipt: response.planReceipt,
    ...(['validated-memory', 'verified-contract'].includes(response.planSource) ? { planSource: response.planSource } : {}),
  }
}
...
export async function repairQueryPlan({ question, failedPlan, validationError, catalog, knowledge, semanticContext, sourceDatabase }) {
  ...
  return enforceSemanticGuards({
    ...
    // A repaired plan gets its own host receipt and source. Never carry forward
    // a failed cached plan's receipt or reuse marker.
    ...hostPlanMetadata(response),
  }, question, catalog, sourceDatabase)
}
...
// Call only after the host metric contract has passed structural validation.
// Its two selected expressions identify the group and measure even if either
// output is numeric or the model omitted presentation fields entirely.
export function deriveValidatedLocalPresentation(ast, contract, providerCode, rows = []) {
  if (String(providerCode || '').toLowerCase() !== 'localopenaicompatible'
    || contract?.version !== 'simple-count-v1'
    || !['count_records', 'sum_values', 'average_values'].includes(contract.measure)) return null
  const group = ast.columns.find(item => item.expr?.type === 'column_ref')
  const measure = ast.columns.find(item => item !== group)
  const groupName = group.as || group.expr.column
  const fields = [...new Set(rows.flatMap(row => Object.keys(row || {})))]
  const labelField = fields.find(field => field.toLowerCase() === groupName.toLowerCase()) || groupName
  // Unaliased function names are rendered by MySQL. Bind the sole non-group
  // result column rather than guessing its quoting or expression spelling.
  const valueFields = measure.as
    ? fields.filter(field => field.toLowerCase() === measure.as.toLowerCase())
    : fields.filter(field => field !== labelField)
  return { labelField, valueFields, validatedValueFields: valueFields,
    chartType: contract.measure === 'average_values' ? 'bar' : 'auto' }
}

export function createVerifiedDataInsights({ rows, chart }) {
  const display = value => value == null ? 'not available' : String(value).replace(/\s+/g, ' ').slice(0, 120)
  const fields = [...new Set([chart.labelField, ...chart.valueFields])].filter(Boolean).slice(0, 4)
  const excerpt = rows.slice(0, 3).map(row => fields
    .map(field => `${display(field)}=${display(row?.[field])}`).join(', ')).filter(Boolean).join('; ')
  return {
    summary: rows.length
      ? `The fresh query returned ${rows.length} result group${rows.length === 1 ? '' : 's'}. ${rows.length > 3 ? 'First 3 groups: ' : ''}${excerpt ? `${excerpt}. ` : ''}This summary is computed from current query rows.`
      : 'The fresh query returned no matching records in this scope. This does not establish zero activity or complete source coverage. This summary is computed from current query rows.',
    insights: [],
    summarySource: 'verified-data',
  }
}
