...
builder.Services.AddSingleton<InternalInterviewTransport>();
builder.Services.AddSingleton<IInternalInterviewLanguageModel, InternalInterviewLanguageModel>();
builder.Services.AddSingleton<InternalInterviewAiService>();
builder.Services.AddSingleton<InternalInterviewMediaStore>();
builder.Services.AddSingleton<InternalInterviewSpeechService>();
builder.Services.AddSingleton<InternalInterviewInvitationService>();
if (builder.Configuration.GetValue("BackgroundWorkers:Enabled", true))
{
    builder.Services.AddHostedService<InternalInterviewWorker>();
    builder.Services.AddHostedService<InternalInterviewAiWorker>();
}
...
{
    var portableAttachmentRoot = attachmentDataRoot.Replace('\\', '/');
    if (portableAttachmentRoot.Equals("/app", StringComparison.OrdinalIgnoreCase) ||
        portableAttachmentRoot.StartsWith("/app/", StringComparison.OrdinalIgnoreCase))
    {
        var relativeAttachmentRoot = portableAttachmentRoot.Length == 4 ? "" : portableAttachmentRoot[5..];
        attachmentDataRoot = string.IsNullOrWhiteSpace(relativeAttachmentRoot)
            ? builder.Environment.ContentRootPath
            : Path.Combine(builder.Environment.ContentRootPath, relativeAttachmentRoot.Replace('/', Path.DirectorySeparatorChar));
    }
}
...
builder.Services.AddSingleton<RecruitmentLifecycleNotificationService>();
...
if (args.Any(arg => arg.Equals("--migrate-internal-interviews", StringComparison.OrdinalIgnoreCase)))
{
    ...
    app.Logger.LogInformation("Additive internal interview migration completed. Existing hiring records unchanged.");
    return;
}

if (args.Contains("--migrate-recruitment-journey"))
{
    await app.Services.GetRequiredService<RecruitmentTalentRepository>().InitializeJourneySchemaAsync();
    app.Logger.LogInformation("Recruitment journey additive schema upgrade completed.");
    return;
}
...
app.MapPost("/api/workflows/tasks/{taskId:long}/{action}", async (WorkflowRepository repository, EssMssRepository essRepository, PayRunRepository payRuns, RecruitmentRepository recruitment, RecruitmentTalentRepository recruitmentTalent, RecruitmentPipelineRepository recruitmentPipeline, RecruitmentCaseRepository recruitmentCases, RecruitmentCandidateActionRepository candidateActions, RecruitmentPipelineActionService pipelineActions, RecruitmentLifecycleNotificationService lifecycleNotifications, NotificationRepository notifications, NotificationAutomationRepository notificationAutomation,long taskId,string action,WorkflowActionRequest request,HttpContext context) =>
{
    ...
    {
        await recruitment.SyncWorkflowStatusAsync(instance.ResourceId,instance.Status,user.Id);
        if(instance.Status=="Approved" && long.TryParse(instance.ResourceId,out var approvedRequisitionId))
        {
            var (_, launchError) = await recruitmentPipeline.EnsureApprovedRequisitionLaunchAsync(approvedRequisitionId,user,true);
            if(launchError.Length>0)return Results.Conflict(new{error=launchError});
        }
    }
    if(instance?.ResourceType=="RecruitmentBudget" && long.TryParse(instance.ResourceId,out var budgetRequisitionId))
        await recruitment.SyncBudgetApprovalAsync(budgetRequisitionId,instance.Id,instance.Status);
    if(instance?.ResourceType=="RecruitmentOffer")
    {
        await recruitmentTalent.SyncOfferWorkflowStatusAsync(instance.ResourceId,instance.Status,user,instance.Id);
        if(long.TryParse(instance.ResourceId,out var workflowOfferId))
        {
            await recruitmentCases.AdvanceHiringCaseForOfferMilestoneAsync(workflowOfferId,user);
            if(instance.Status is "Approved" or "Rejected")
                await lifecycleNotifications.QueueOfferMilestoneAsync(workflowOfferId,instance.Status,user);
        }
    }
    if(instance?.ResourceType==RecruitmentTalentRepository.FinalOfferResource && instance.Status=="Approved"
        && long.TryParse(instance.ResourceId,out var finalOfferId))
    {
        var (_, finalError) = await recruitmentTalent.GenerateFinalOfferAsync(finalOfferId,instance.Id,user,context.RequestAborted);
        if(finalError.Length>0)return Results.Conflict(new { error="Approval saved. Final PDF needs retry: " + finalError });
        var finalOffer = (await recruitmentTalent.GetOffersAsync(user, null)).FirstOrDefault(row => row.Id == finalOfferId);
        if(finalOffer is not null) await ApplyRecruitmentDecisionAsync(finalOffer.ApplicationId,"DocumentsVerified",user,recruitmentPipeline,pipelineActions,candidateActions,recruitmentCases);
    }
    ...
    {
        var (approvedCase, hiringCaseError) = await recruitmentCases.SyncHiringCaseAdvanceWorkflowStatusAsync(hiringCaseAdvanceRequestId,instance.Status,user);
        if(hiringCaseError.Length>0)return Results.Conflict(new{error=hiringCaseError});
        // Approval applies exactly its requested transition, then re-evaluates the next
        // stage's own evidence gates. Never require a second manual click to resume it.
        if(instance.Status=="Approved" && approvedCase is not null)
            await recruitmentCases.AdvanceHiringCaseAutomationAsync(approvedCase.Id,user);
    }
    else if(instance?.ResourceType=="RecruitmentPipelineTransition" && long.TryParse(instance.ResourceId,out var transitionRequestId))
    {
        ...
        if(transition.Result?.Status=="Applied")
        {
            ...
            await recruitmentCases.AdvanceHiringCaseForCandidateMilestoneAsync(transition.Result.ApplicationId,"ProfilesSelected",user);
        }
    }
    if(instance?.ResourceType=="RecruitmentPipelineStageAction")
    {
        ...
        if(completion.Approved && completion.ApplicationId>0)
        {
            ...
            if(completion.ResumeSubmission)
            {
                var submissionActions = await pipelineActions.ExecuteAsync(completion.ApplicationId,"OnSubmission",user,completion.StageInstanceId);
                if(!submissionActions.HasBlockingFailure)
                    await ApplyRecruitmentDecisionAsync(completion.ApplicationId,"Submitted",user,recruitmentPipeline,pipelineActions,candidateActions,recruitmentCases,completion.StageInstanceId);
            }
            else
            {
                // Idempotent entry actions resume only this still-current stage; an old
                // approval callback must never trigger an unrelated later stage.
                await pipelineActions.ExecuteAsync(completion.ApplicationId,"OnEntry",user,completion.StageInstanceId);
            }
        }
    }
...
app.MapPost("/api/ess/recruitment/requisitions/{id:long}/submit", async (RecruitmentRepository repository, RecruitmentPipelineRepository recruitmentPipeline, WorkflowRepository workflows, long id, HttpContext context) =>
{
    var user = CurrentUser(context);
    if (!RecruitmentRepository.HasRecruitmentCreateAccess(user)) return Results.StatusCode(403);
    ...
    {
        var (_, launchError) = await recruitmentPipeline.EnsureApprovedRequisitionLaunchAsync(row.Id, user, true);
        if (launchError.Length > 0) return Results.Conflict(new { error = launchError });
    }
...
app.MapGet("/api/recruitment/budget-approvers", async (RecruitmentRepository repository, int clientId, HttpContext context) =>
    HasPermission(context,"recruitment.manage") || HasPermission(context,"settings.manage")
        ? Results.Ok(await repository.GetBudgetApproversAsync(clientId,CurrentUser(context))) : Results.StatusCode(403));
app.MapGet("/api/recruitment/clients/{clientId:int}/final-offer-settings", async (RecruitmentTalentRepository repository, int clientId, HttpContext context) =>
{
    if (!IsSuperAdmin(context)) return Results.StatusCode(403);
    var row = await repository.GetFinalOfferSettingsAsync(clientId,CurrentUser(context));
    return row is null ? Results.NotFound() : Results.Ok(row);
});
app.MapPost("/api/recruitment/clients/{clientId:int}/final-offer-settings", async (RecruitmentTalentRepository repository, int clientId, SaveRecruitmentFinalOfferSettings request, HttpContext context) =>
{
    if (!IsSuperAdmin(context)) return Results.StatusCode(403);
    var (row,error) = await repository.SaveFinalOfferSettingsAsync(clientId,request,CurrentUser(context));
    return row is null ? Results.BadRequest(new { error }) : Results.Ok(row);
});
...
app.MapPost("/api/recruitment/requisitions/{id:long}/submit", async (RecruitmentRepository repository, RecruitmentPipelineRepository recruitmentPipeline, WorkflowRepository workflows, long id, HttpContext context) =>
{
    if (!HasPermission(context, "recruitment.manage") && !HasPermission(context, "settings.manage")) return Results.StatusCode(403);
    var user = CurrentUser(context);
    var (row, error) = await repository.SubmitAsync(id, user, workflows);
...
app.MapPost("/api/recruitment/requisitions/{id:long}/ensure-job", async (RecruitmentPipelineRepository repository, long id, HttpContext context) =>
{
    if (!HasPermission(context, "settings.manage")) return Results.StatusCode(403);
    var (row, error) = await repository.EnsureApprovedRequisitionLaunchAsync(id, CurrentUser(context), true);
    return row is null ? Results.BadRequest(new { error }) : Results.Ok(row);
});
...
app.MapPost("/api/recruitment/work-orders", async (RecruitmentCaseRepository repository, SaveRecruitmentWorkOrder request, HttpContext context) =>
{
    ...
    var user = CurrentUser(context);
    var (row, error) = await repository.SaveWorkOrderAsync(request, user);
    if (row is not null)
    {
        var (cases, launchError) = await repository.EnsureHiringCasesForWorkOrderAsync(row.Id, user);
        row = await repository.GetWorkOrderAsync(row.Id, user);
        if (row is not null) row.SlaLaunchMessage = launchError.Length > 0
            ? launchError
            : cases.Count > 0 ? "Work-order SLA started from the received timestamp." : "";
    }
    return row is null ? Results.BadRequest(new { error }) : Results.Ok(row);
});
...
{
    if (!HasPermission(context, "recruitment.hiring-case.manage") && !HasPermission(context, "recruitment.manage") && !HasPermission(context, "settings.manage")) return Results.StatusCode(403);
    var (row, error) = await repository.AdvanceHiringCaseAsync(id, request, CurrentUser(context), manual: true);
    return row is null ? Results.BadRequest(new { error }) : Results.Ok(row);
});
...
app.MapPost("/api/recruitment/process-documents", async (RecruitmentCaseRepository repository, RecruitmentLifecycleNotificationService lifecycleNotifications, SaveRecruitmentProcessDocument request, HttpContext context) =>
{
    ...
    var user = CurrentUser(context);
    var (row, error) = await repository.SaveProcessDocumentAsync(request, user);
    if (row?.Status == "Signed" && row.HiringCaseId is > 0)
    {
        await repository.AdvanceHiringCaseForDocumentMilestoneAsync(row.HiringCaseId.Value, user);
        await lifecycleNotifications.QueueMomSignedAsync(row.Id, user);
    }
    return row is null ? Results.BadRequest(new { error }) : Results.Ok(row);
});
...
app.MapPost("/api/recruitment/process-documents/{id:long}/signatures", async (RecruitmentCaseRepository repository, RecruitmentLifecycleNotificationService lifecycleNotifications, long id, SaveRecruitmentProcessDocumentSignature request, HttpContext context) =>
{
    ...
    if (row is not null)
    {
        var document = (await repository.ListProcessDocumentsAsync(user, null, null)).FirstOrDefault(item => item.Id == id);
        if (document?.Status == "Signed" && document.HiringCaseId is > 0)
        {
            await repository.AdvanceHiringCaseForDocumentMilestoneAsync(document.HiringCaseId.Value, user);
            await lifecycleNotifications.QueueMomSignedAsync(id, user);
        }
    }
    return row is null ? Results.BadRequest(new { error }) : Results.Ok(row);
});
...
app.MapGet("/api/recruitment/open-positions", async (RecruitmentRepository repository, int? clientId, HttpContext context) =>
    RecruitmentPermissions.Has(CurrentUser(context), "recruitment.interview.schedule", "recruitment.offer.manage", "recruitment.proposal.manage", "recruitment.position.view", "recruitment.position.manage", "recruitment.work-order.manage") ? Results.Ok(await repository.OpenPositionsAsync(CurrentUser(context), clientId)) : Results.StatusCode(403));
...
app.MapGet("/api/recruitment/candidates", async (RecruitmentTalentRepository repository, int? clientId, string? query, string? status, HttpContext context) =>
    RecruitmentPermissions.Has(CurrentUser(context), "recruitment.candidate.view", "recruitment.offer.manage", "recruitment.proposal.manage")
...
app.MapGet("/api/recruitment/applications", async (RecruitmentTalentRepository repository, long? positionId, long? candidateId, string? stage, bool? negotiationOnly, int? clientId, HttpContext context) =>
    RecruitmentPermissions.Has(CurrentUser(context), "recruitment.interview.schedule", "recruitment.offer.manage", "recruitment.proposal.manage", "recruitment.candidate.view")
...
{
    if (!HasPermission(context, "settings.manage")) return Results.StatusCode(403);
    var (ok, error) = await repository.DeleteApplicationCompletelyAsync(id, CurrentUser(context),
        context.Connection.RemoteIpAddress?.ToString() ?? "", context.Request.Headers.UserAgent.ToString(), context.RequestAborted);
    return ok ? Results.NoContent() : Results.BadRequest(new { error });
});
...
app.MapPost("/api/recruitment/interviews/direct-application", async (RecruitmentTalentRepository repository, RecruitmentPipelineRepository pipelines, RecruitmentPipelineActionService actions, RecruitmentCandidateActionRepository candidateActions, DirectInterviewApplication request, HttpContext context) =>
{
    var user = CurrentUser(context);
    var (row, error) = await repository.CreateDirectInterviewApplicationAsync(request, user);
    if (row is null) return Results.BadRequest(new { error });
    var (_, pipelineError) = await EnsureRecruitmentApplicationAutomationAsync(row.Id, user, pipelines, actions, candidateActions);
    row.PipelineWarning = pipelineError;
    return Results.Ok(row);
});
app.MapGet("/api/recruitment/applications/{id:long}/negotiation", async (RecruitmentTalentRepository repository, long id, HttpContext context) =>
{
    var user = CurrentUser(context);
    if (!RecruitmentPermissions.Has(user, "recruitment.offer.manage", "recruitment.proposal.manage")) return Results.StatusCode(403);
    var row = await repository.GetNegotiationAsync(id, user);
    return row is null ? Results.NotFound() : Results.Ok(row);
});
app.MapPost("/api/recruitment/applications/{id:long}/negotiation", async (RecruitmentTalentRepository repository, RecruitmentCaseRepository cases, long id, SaveRecruitmentNegotiation request, HttpContext context) =>
{
    var user = CurrentUser(context);
    var (row, error) = await repository.SaveNegotiationAsync(id, request, user);
    if (row is null) return Results.BadRequest(new { error });
...
app.MapPost("/api/recruitment/interviews/{id:long}/link-application", async (RecruitmentTalentRepository repository, RecruitmentPipelineRepository pipelines, RecruitmentCaseRepository hiringCases, RecruitmentCandidateActionRepository candidateActions, RecruitmentPipelineActionService pipelineActions, long id, JsonElement request, HttpContext context) =>
{
    var user = CurrentUser(context);
    if (!request.TryGetProperty("applicationId", out var property) || !property.TryGetInt64(out var applicationId)) return Results.BadRequest(new { error = "Select the candidate's application." });
    var (row, error) = await repository.LinkDirectInterviewAsync(id, applicationId, user);
    if (row is null) return Results.BadRequest(new { error });
    row.PipelineTransitionMessage = await pipelines.EnterDirectInterviewAsync(applicationId, user);
    if (row.Status == "Completed" && row.Result is "Selected" or "Rejected")
        row.PipelineTransitionMessage = await ApplyRecruitmentDecisionAsync(applicationId, row.Result, user, pipelines, pipelineActions, candidateActions, hiringCases);
    return Results.Ok(row);
});
...
{
    var user = CurrentUser(context);
    if (user.ClientId is not null || !user.Roles.Contains("super_admin", StringComparer.OrdinalIgnoreCase)) return Results.StatusCode(403);
...
app.MapPost("/api/recruitment/interviews", async (RecruitmentTalentRepository repository, RecruitmentPipelineRepository pipelines, RecruitmentCaseRepository hiringCases, RecruitmentCandidateActionRepository candidateActions, RecruitmentPipelineActionService pipelineActions, InternalInterviewRepository internalInterviews, SaveRecruitmentInterview request, HttpContext context) =>
{
    ...
    if (row is { ApplicationId: > 0, IsStandalone: true } && RecruitmentPermissions.Has(user, "recruitment.interview.schedule"))
        row.PipelineTransitionMessage = await pipelines.EnterDirectInterviewAsync(row.ApplicationId, user);
...
app.MapPost("/api/recruitment/interviews/{id:long}/invite", async (RecruitmentTalentRepository repository, NotificationRepository notifications, InternalInterviewInvitationService internalInvitations, long id, HttpContext context, CancellationToken cancellationToken) =>
{
    if (!HasPermission(context, "recruitment.interview.schedule") && !HasPermission(context, "recruitment.manage") && !HasPermission(context, "settings.manage")) return Results.StatusCode(403);
    var user = CurrentUser(context);
    var (interview, error) = await repository.GetInterviewInvitationAsync(id, user);
    if (interview is null) return Results.BadRequest(new { error });
    if (string.IsNullOrWhiteSpace(interview.CandidateEmail)) return Results.BadRequest(new { error = "The candidate email is missing. Update the candidate profile before sending the invite." });
    InternalInterviewInvitation? internalInvite;
    try { internalInvite = await internalInvitations.ResolveAsync(id, user); }
    catch (InternalInterviewException issue) { return Results.Json(new { error = issue.Message }, statusCode: issue.StatusCode); }
    if (internalInvite is null && (string.IsNullOrWhiteSpace(interview.LocationOrLink) || interview.LocationOrLink == "Internal HRMS interview"))
        return Results.BadRequest(new { error = "Configure the internal interview or add an external meeting/location before sending the invite." });

    static string Safe(string value) => System.Net.WebUtility.HtmlEncode(value ?? "");
    var schedule = $"{interview.ScheduledStart:dd MMM yyyy, hh:mm tt} - {interview.ScheduledEnd:hh:mm tt} ({Safe(interview.TimeZoneId)})";
    var destinationLabel = interview.Mode.Equals("Virtual", StringComparison.OrdinalIgnoreCase) ? "Meeting link" : "Location / contact";
    var destination = interview.LocationOrLink.StartsWith("http://", StringComparison.OrdinalIgnoreCase) || interview.LocationOrLink.StartsWith("https://", StringComparison.OrdinalIgnoreCase)
        ? $"<a href=\"{Safe(interview.LocationOrLink)}\">Join interview</a>"
        : Safe(interview.LocationOrLink);
    var subject = $"Interview invitation - {interview.PositionTitle} - {interview.RoundCode}";
    string BuildBody(string recipientDestination) => $"""
<p>Hello,</p>
<p>An interview has been scheduled for <strong>{Safe(interview.CandidateName)}</strong>.</p>
<table style="border-collapse:collapse">
<tr><td style="padding:4px 14px 4px 0"><strong>Position</strong></td><td>{Safe(interview.PositionTitle)}</td></tr>
<tr><td style="padding:4px 14px 4px 0"><strong>Round</strong></td><td>{Safe(interview.RoundCode)} - {Safe(interview.InterviewType)}</td></tr>
<tr><td style="padding:4px 14px 4px 0"><strong>Schedule</strong></td><td>{schedule}</td></tr>
<tr><td style="padding:4px 14px 4px 0"><strong>Mode</strong></td><td>{Safe(interview.Mode)}</td></tr>
<tr><td style="padding:4px 14px 4px 0"><strong>{destinationLabel}</strong></td><td>{recipientDestination}</td></tr>
</table>
<p>Please be available a few minutes before the scheduled time.</p>
""";
    var recipients = new[] { interview.CandidateEmail }.Concat(interview.PanelEmails).Where(value => !string.IsNullOrWhiteSpace(value)).Distinct(StringComparer.OrdinalIgnoreCase).ToArray();
    var sent = new List<string>();
    foreach (var recipient in recipients)
    {
        var recipientUrl = internalInvite is null ? null : recipient.Equals(interview.CandidateEmail, StringComparison.OrdinalIgnoreCase) ? internalInvite.CandidateUrl : internalInvite.PanelUrl;
        var body = BuildBody(recipientUrl is null ? destination : $"<a href=\"{Safe(recipientUrl)}\">Open secure internal interview</a><br/>" +
            (recipient.Equals(interview.CandidateEmail, StringComparison.OrdinalIgnoreCase) ? "Candidate-only expiring link. Please do not forward it. Review the consent notice before joining." : "Panel members must sign in with their existing HRMS account. This is not the candidate credential."));
        var (delivered, sendError) = await notifications.SendDirectAsync(recipient, subject, body, new NotificationEvent
        {
            EventCode = "RECRUITMENT_INTERVIEW_INVITE",
            ResourceType = "RecruitmentInterview",
            ResourceId = id.ToString(System.Globalization.CultureInfo.InvariantCulture),
            ClientId = interview.ClientId,
            ActorUserId = user.Id,
            ActorName = user.DisplayName,
            ActorEmail = user.Email,
            PayloadJson = JsonSerializer.Serialize(new { interview.InterviewId, interview.CandidateName, interview.PositionTitle, interview.ScheduledStart, interview.ScheduledEnd, interview.Mode })
        }, cancellationToken);
        if (!delivered) return Results.BadRequest(new { error = sendError, sentRecipients = sent.Count });
        sent.Add(recipient);
    }
    return Results.Ok(new { recipientCount = sent.Count });
});
...
app.MapGet("/api/recruitment/offers", async (RecruitmentTalentRepository repository, long? applicationId, HttpContext context) =>
    RecruitmentPermissions.Has(CurrentUser(context), "recruitment.offer.manage", "recruitment.offer.issue", "recruitment.proposal.manage") ? Results.Ok(await repository.GetOffersAsync(CurrentUser(context), applicationId)) : Results.StatusCode(403));
...
{
    if (!RecruitmentPermissions.Has(CurrentUser(context), "recruitment.offer.manage", "recruitment.proposal.manage")) return Results.StatusCode(403);
...
{
    if (!RecruitmentPermissions.Has(CurrentUser(context), "recruitment.offer.issue")) return Results.StatusCode(403);
    var (row, error) = await repository.GenerateOfferLetterAsync(id, CurrentUser(context),
        context.Connection.RemoteIpAddress?.ToString() ?? "", context.Request.Headers.UserAgent.ToString(), context.RequestAborted);
    return row is null ? Results.BadRequest(new { error }) : Results.Ok(row);
});
app.MapPost("/api/recruitment/offers/{id:long}/final-letter", async (RecruitmentTalentRepository repository, long id, HttpContext context) =>
{
    if (!RecruitmentPermissions.Has(CurrentUser(context), "recruitment.offer.issue")) return Results.StatusCode(403);
    var (row,error) = await repository.RequestFinalOfferAsync(id,CurrentUser(context),context.RequestAborted);
    return row is null ? Results.BadRequest(new { error }) : Results.Ok(row);
});
app.MapPost("/api/recruitment/offers/{id:long}/status", async (RecruitmentTalentRepository repository, RecruitmentPipelineRepository pipelines, RecruitmentCandidateActionRepository candidateActions, RecruitmentPipelineActionService pipelineActions, RecruitmentCaseRepository hiringCases, RecruitmentLifecycleNotificationService lifecycleNotifications, long id, JsonElement request, HttpContext context) =>
{
    if (!RecruitmentPermissions.Has(CurrentUser(context), "recruitment.offer.manage", "recruitment.offer.issue", "recruitment.proposal.manage")) return Results.StatusCode(403);
    var user = CurrentUser(context);
    var status = request.TryGetProperty("status", out var value) ? value.GetString() ?? "" : "";
    var requiredOfferPermissions = status is "Pending Candidate" or "Released" or "Accepted" ? new[] { "recruitment.offer.issue" } : status is "Negotiation" or "Pending Approval" ? new[] { "recruitment.offer.manage", "recruitment.proposal.manage" } : new[] { "recruitment.offer.manage" };
    if (!RecruitmentPermissions.Has(user, requiredOfferPermissions)) return Results.StatusCode(403);
    ...
    if (row is not null)
        await hiringCases.AdvanceHiringCaseForOfferMilestoneAsync(row.Id, user);
    RecruitmentCandidateActionSession? candidateAction = null;
    if (row?.Status == "Pending Candidate")
    {
        ...
        (candidateAction, _) = await candidateActions.EnsureForCurrentStageAsync(row.ApplicationId, user);
    }
    if (row?.Status is "Accepted" or "Rejected" or "Withdrawn")
        row.PipelineTransitionMessage = await ApplyRecruitmentDecisionAsync(row.ApplicationId, row.Status, user, pipelines, pipelineActions, candidateActions, hiringCases);
    if (row?.Status is "Pending Candidate" or "Negotiation" or "Accepted" or "Rejected")
        await lifecycleNotifications.QueueOfferMilestoneAsync(row.Id, row.Status, user, candidateAction?.ActionToken ?? "");
    if (row?.Status == "Accepted") await repository.TryRequestAcceptedFinalOfferAsync(row.Id,user);
    return row is null ? Results.BadRequest(new { error }) : Results.Ok(row);
});
app.MapPost("/api/recruitment/applications/{applicationId:long}/checklist/{itemId:long}/complete", async (RecruitmentTalentRepository repository, RecruitmentPipelineRepository pipelines, RecruitmentCandidateActionRepository candidateActions, RecruitmentPipelineActionService pipelineActions, RecruitmentCaseRepository hiringCases, long applicationId, long itemId, JsonElement request, HttpContext context) =>
{
    if (!HasPermission(context, "recruitment.manage") && !HasPermission(context, "settings.manage")) return Results.StatusCode(403);
    ...
    var user = CurrentUser(context);
    var (row, error) = await repository.CompleteChecklistItemAsync(applicationId, itemId, publicId, user);
    if (row is not null)
        await ApplyRecruitmentDecisionAsync(applicationId, "DocumentsVerified", user, pipelines, pipelineActions, candidateActions, hiringCases);
    return row is null ? Results.BadRequest(new { error }) : Results.Ok(row);
});
app.MapPost("/api/recruitment/applications/{applicationId:long}/convert-to-employee", async (RecruitmentTalentRepository repository, RecruitmentPipelineRepository pipelines, RecruitmentCandidateActionRepository candidateActions, RecruitmentPipelineActionService pipelineActions, RecruitmentCaseRepository hiringCases, long applicationId, ConvertCandidateToEmployeeRequest request, HttpContext context) =>
{
    if (!HasPermission(context, "employees.manage") && !HasPermission(context, "settings.manage")) return Results.StatusCode(403);
    var user = CurrentUser(context);
    var (row, error) = await repository.ConvertToEmployeeAsync(applicationId, request, user);
    if (row is not null)
...
recruitmentOrchestration.MapPost("/job-postings/{id:long}/auto-run-ats", async (RecruitmentPipelineRepository repository, RecruitmentTalentRepository talent, PublicPortalUrlResolver publicPortalUrls, long id, UpdateRecruitmentJobPostingAtsRequest request, HttpContext context) =>
{
    if (!HasRecruitmentManagement(context)) return Results.StatusCode(403);
    var user = CurrentUser(context);
    var (row, error) = await repository.UpdateJobPostingAtsAsync(id, request, user);
    if (row is null) return Results.BadRequest(new { error });
    if (row.AtsCatchUpRequired)
    {
        foreach (var applicationId in await repository.GetJobPostingApplicationIdsAsync(id, user))
            await talent.QueuePolicyScoreAsync(applicationId, user);
    }
...
recruitmentOrchestration.MapPost("/pipeline-versions/{versionId:long}/publish", async (RecruitmentPipelineRepository repository, long versionId, HttpContext context) =>
{
    ...
    if (row is not null)
    {
        try { row.RuntimeUpdate = await repository.SynchronizePublishedRevisionsAsync(CurrentUser(context), row.PipelineDefinitionId); }
        catch (Exception exception)
        {
            context.RequestServices.GetRequiredService<ILoggerFactory>().CreateLogger("PipelineRevisions")
                .LogError(exception, "Published pipeline {VersionId}; runtime synchronization will retry in the worker.", versionId);
            row.RuntimeUpdate = new(0, 0, ["Published successfully. Runtime updates are queued for retry; check pipeline history before continuing."]);
        }
    }
    return row is null ? Results.BadRequest(new { error }) : Results.Ok(row);
});
...
recruitmentOrchestration.MapPost("/applications/{applicationId:long}/transitions/{transitionId:long}", async (RecruitmentPipelineRepository repository, RecruitmentCaseRepository hiringCases, RecruitmentCandidateActionRepository candidateActions, RecruitmentPipelineActionService actions, long applicationId, long transitionId, RecruitmentPipelineTransitionRequest request, HttpContext context) =>
{
    if (!HasRecruitmentManagement(context)) return Results.StatusCode(403);
    var user = CurrentUser(context);
    var selectedTransition = (await repository.GetAvailableTransitionsAsync(applicationId, user))
        .FirstOrDefault(item => item.Id == transitionId);
    var dispositionTransition = selectedTransition is not null
        && (selectedTransition.OutcomeCode.Contains("REJECT", StringComparison.OrdinalIgnoreCase)
            || selectedTransition.OutcomeCode.Contains("WITHDRAW", StringComparison.OrdinalIgnoreCase)
            || selectedTransition.ToStageCode.Contains("REJECT", StringComparison.OrdinalIgnoreCase)
            || selectedTransition.ToStageCode.Contains("WITHDRAW", StringComparison.OrdinalIgnoreCase));
    var currentActions = dispositionTransition
        ? new RecruitmentStageActionExecutionResult { ApplicationId = applicationId, TriggerEvent = "OnEntry" }
        : await actions.ExecuteAsync(applicationId, "OnEntry", user);
    if (!dispositionTransition && currentActions.HasBlockingFailure)
    {
...
app.MapPost("/api/public/recruitment/actions/{token}", async (RecruitmentCandidateActionRepository repository, RecruitmentTalentRepository talent, RecruitmentPipelineRepository pipelines, RecruitmentPipelineActionService pipelineActions, RecruitmentCaseRepository hiringCases, RecruitmentLifecycleNotificationService lifecycleNotifications, string token, CompletePublicCandidateActionRequest request, HttpContext context) =>
{
    ...
    if (row.Status.Equals("NEGOTIATION_REQUESTED", StringComparison.OrdinalIgnoreCase))
        await talent.ApplyNegotiationSlaExtensionForApplicationAsync(row.ApplicationId, request.Remarks, systemUser);
    if (row?.ShouldResumePipeline == true)
    {
        var submissionActions = await pipelineActions.ExecuteAsync(row.ApplicationId, "OnSubmission", systemUser, row.PipelineStageInstanceId);
        if (request.Decision.Trim().Equals("Accepted", StringComparison.OrdinalIgnoreCase)
            || request.Decision.Trim().Equals("Rejected", StringComparison.OrdinalIgnoreCase))
            await ApplyRecruitmentDecisionAsync(row.ApplicationId, request.Decision, systemUser, pipelines, pipelineActions, repository, hiringCases, row.PipelineStageInstanceId);
        else if (string.IsNullOrWhiteSpace(request.Decision) && !submissionActions.HasBlockingFailure)
        {
            var transitionMessage = await ApplyRecruitmentDecisionAsync(row.ApplicationId, "Submitted", systemUser, pipelines, pipelineActions, repository, hiringCases, row.PipelineStageInstanceId);
            if (!string.IsNullOrWhiteSpace(transitionMessage)) row.Message = $"{row.Message} {transitionMessage}";
        }
    }
    if (row!.Status is "NEGOTIATION_REQUESTED" or "ACCEPTED" or "REJECTED")
        await lifecycleNotifications.QueueOfferMilestoneForApplicationAsync(row.ApplicationId,
            row.Status == "NEGOTIATION_REQUESTED" ? "Negotiation" : row.Status == "ACCEPTED" ? "Accepted" : "Rejected", systemUser);
    if (row.Status == "ACCEPTED") await talent.TryRequestAcceptedFinalOfferForApplicationAsync(row.ApplicationId,systemUser);
    return Results.Ok(row);
});
...
app.MapGet("/api/notifications/recruitment-triggers", async (RecruitmentLifecycleNotificationService service, int? clientId, HttpContext context) =>
{
    if (!HasPermission(context, "settings.manage")) return Results.StatusCode(StatusCodes.Status403Forbidden);
    return Results.Ok(await service.ListRulesAsync(CurrentUser(context), clientId));
});
app.MapPost("/api/notifications/recruitment-triggers", async (RecruitmentLifecycleNotificationService service, SaveRecruitmentLifecycleNotificationRule request, HttpContext context) =>
{
    if (!HasPermission(context, "settings.manage")) return Results.StatusCode(StatusCodes.Status403Forbidden);
    try { return Results.Ok(await service.SaveRuleAsync(request, CurrentUser(context))); }
    catch (Exception exception) { return Results.BadRequest(new { error = exception.Message }); }
});
app.MapDelete("/api/notifications/recruitment-triggers/{id:long}", async (RecruitmentLifecycleNotificationService service, long id, HttpContext context) =>
{
    if (!HasPermission(context, "settings.manage")) return Results.StatusCode(StatusCodes.Status403Forbidden);
    return await service.DeleteRuleAsync(id, CurrentUser(context)) ? Results.NoContent() : Results.NotFound();
});
...
app.MapGet("/api/employees/manager-users", async (EmployeeRepository repository, HttpContext context) =>
    HasPermission(context, "employees.view") || HasPermission(context, "employees.manage") || RecruitmentPermissions.Has(CurrentUser(context), "recruitment.interview.schedule")
...
static async Task<string> ApplyRecruitmentDecisionAsync(long applicationId, string decision, AuthUser user,
    RecruitmentPipelineRepository pipelines, RecruitmentPipelineActionService actions, RecruitmentCandidateActionRepository candidateActions,
    RecruitmentCaseRepository hiringCases,
    long? expectedStageInstanceId = null)
{
    var (result, error) = await pipelines.AdvanceApplicationForDecisionAsync(applicationId, decision, user, expectedStageInstanceId);
...
static async Task RunDatabaseSetupAsync(IServiceProvider services, IConfiguration configuration)
{
    ...
    await scopedServices.GetRequiredService<RecruitmentLifecycleNotificationService>().InitializeAsync();
