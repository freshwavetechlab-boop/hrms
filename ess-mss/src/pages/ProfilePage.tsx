...
export function ProfilePage({ user }: { user: User }) {
  ...
  return <section className="profile-page"><div className="profile-head"><span>{initials(`${profile.firstName} ${profile.lastName}`)}</span><div><h3>{`${profile.firstName} ${profile.lastName}`.trim()}</h3></div></div><div className="profile-grid">{fields.map(([label, value]) => <div key={label}><span>{label}</span><b>{value}</b></div>)}</div><div className="empty-work"><b>Need to update your details?</b><span>Personal and payment detail changes will be submitted through a controlled HR verification workflow in the next release.</span></div></section>
}
