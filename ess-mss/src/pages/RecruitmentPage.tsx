...
import type { AttachmentFieldConfiguration, LoadState, RecruitmentDashboard, RecruitmentEmployeeReferral, RecruitmentInternalOpening, RecruitmentOptions, RecruitmentRequisition, SaveEmployeeReferral, SaveRecruitmentRequisition, User, WorkflowTrail } from '../types'
...
export function RecruitmentPage({ user }: { user: User }) {
  ...
  const [referralDocument, setReferralDocument] = useState<AttachmentFieldConfiguration | null>(null)
  ...
  useEffect(() => { if (user.clientId) void essApi.recruitmentAttachmentConfigurations(user.clientId).then(rows => setReferralDocument(rows.find(row => row.fieldKey === 'RESUME') ?? rows[0] ?? null)) }, [user.clientId])
  ...
  const submitReferral = async (event: FormEvent) => {
    event.preventDefault()
    if (!referral.candidateEmail.trim() && !referral.candidatePhone.trim()) { showToast('Candidate email or phone is required.', 'error'); return }
    if (referralDocument?.isRequired && !referralResume) { showToast('Select the candidate resume.', 'error'); return }
    try {
      const saved = await essApi.submitRecruitmentReferral(referral)
      let resumeUploaded = !referralResume
      if (referralResume && referralDocument) {
        try { await essApi.uploadReferralResume(saved.id, referralDocument.id, referralResume); resumeUploaded = true }
        catch { resumeUploaded = false }
      }
      showToast(resumeUploaded ? 'Referral and resume submitted.' : 'Referral saved, but resume upload failed. Retry it from My referrals.', resumeUploaded ? 'success' : 'error'); setReferral({ positionId: 0, candidateName: '', candidateEmail: '', candidatePhone: '', relationship: '', remarks: '' }); setReferralResume(null); void load()
    }
    catch (error) { showToast(error instanceof Error ? error.message : 'Unable to submit referral.', 'error') }
  }
  const uploadExistingReferralResume = async (referralId: number, file: File | null) => {
    if (!file || !referralDocument) return
    try { await essApi.uploadReferralResume(referralId, referralDocument.id, file); showToast('Referral resume uploaded.', 'success') }
    catch (error) { showToast(error instanceof Error ? error.message : 'Unable to upload referral resume.', 'error') }
  }
