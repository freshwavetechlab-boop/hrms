...
export function LeavePage({ user }: { user: User }) {
  ...
  const apply = async (event: FormEvent) => {
    ...
    try {
      ...
      setFromDate(''); setToDate(''); setDayType('Full Day'); setReason(''); setDrawer(false)
      void load()
    } catch (e) {
  ...
  return <section className="leave-workspace">{state === 'loading' && <div className="empty-work"><span>Loading your leave balances...</span></div>}{state === 'error' && <div className="empty-work"><b>Leave data is unavailable.</b><span>Contact HR for assistance.</span></div>}{state === 'ready' && <><div className="leave-toolbar"><button type="button" onClick={() => setDrawer(true)}>Apply leave</button></div><div className="balance-grid">{rows.map(row => <article key={`${row.leaveCode}-${row.balanceDate}`}><span>{row.leaveCode}</span><strong>{row.balance}</strong><b>{row.leaveType}</b><small>{row.allowHalfDay ? 'Half-day allowed' : 'Full-day only'} / As of {new Date(row.balanceDate).toLocaleDateString('en-IN')}</small></article>)}{!rows.length && <div className="empty-work"><b>No active leave types found.</b><span>Configure active leave types for your client in Settings / Leave Types.</span></div>}</div><section className="request-list"><div className="request-list-head"><h3>My requests</h3><div><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search requests" /><select value={typeFilter} onChange={event => setTypeFilter(event.target.value)}>{types.map(item => <option key={item}>{item}</option>)}</select><select value={statusFilter} onChange={event => setStatusFilter(event.target.value)}>{statuses.map(item => <option key={item}>{item}</option>)}</select></div></div>{filtered.map(item => <article key={item.id}><div><b>{item.leaveType}</b><span>{item.leaveCode} / {item.dayType || 'Full Day'} / {item.days} day(s)</span></div><div><b>{item.fromDate.slice(0, 10)}{item.fromDate.slice(0, 10) !== item.toDate.slice(0, 10) ? ` to ${item.toDate.slice(0, 10)}` : ''}</b><span>{item.reason || 'No reason added'}</span></div><div><b>{item.createdAt ? new Date(item.createdAt).toLocaleString('en-IN') : '-'}</b><span>Created</span></div><small className={`status-pill ${statusClass(item.status)}`}>{item.status}</small><button className="trail-link" type="button" onClick={() => void openTrail(item)}><span aria-hidden="true">i</span>Details</button></article>)}{!filtered.length && <p>No matching leave requests.</p>}</section>{drawer && <div className="drawer-backdrop" onClick={() => setDrawer(false)}><form className="leave-drawer" onSubmit={apply} onClick={event => event.stopPropagation()}><header><h3>Apply leave</h3><button type="button" onClick={() => setDrawer(false)}>x</button></header>{rows.length ? <><label><span>Leave type</span><select value={leaveCode} onChange={event => setLeaveCode(event.target.value)}>{rows.map(item => <option value={item.leaveCode} key={item.leaveCode}>{item.leaveType} ({item.leaveCode}) / {item.balance}</option>)}</select></label><label><span>Duration</span><select value={dayType} onChange={event => setDayType(event.target.value)}><option>Full Day</option>{selectedLeave?.allowHalfDay && <option>First Half</option>}{selectedLeave?.allowHalfDay && <option>Second Half</option>}</select></label><label><span>From date</span><input required type="date" value={fromDate} onChange={event => setFromDate(event.target.value)} /></label>{dayType === 'Full Day' && <label><span>To date</span><input required type="date" value={toDate} onChange={event => setToDate(event.target.value)} /></label>}<label><span>Reason</span><textarea required value={reason} onChange={event => setReason(event.target.value)} placeholder="Brief reason for leave" /></label><button>Submit for approval</button></> : <div className="empty-work"><b>No leave type available.</b><span>Please ask HR to activate Leave Types for your client.</span></div>}</form></div>}{trailRequest && <TrailModal request={trailRequest} trail={trail} onClose={() => { setTrailRequest(null); setTrail(null) }} />}</>}</section>
}
